...
//  val BDP_SERVER_SOCKET_SERVICE_SCAN_PACKAGES = CommonVars("wds.linkis.server.socket.service.scan.packages", BDP_SERVER_RESTFUL_SCAN_PACKAGES.getValue)
  ...
  val BDP_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE = CommonVars("wds.linkis.datasource.hikari.maximumPoolSize", 100)
  val BDP_DATASOURCE_HIKARI_MINIMUM_IDLE = CommonVars("wds.linkis.datasource.hikari.minimumIdle", 10)
}
