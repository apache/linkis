...
-- 变量：
SET
@SPARK_LABEL="spark-2.4.3";
SET
@HIVE_LABEL="hive-2.3.3";
SET
@PYTHON_LABEL="python-python2";
SET
@PIPELINE_LABEL="pipeline-1";
SET
@JDBC_LABEL="jdbc-4";
SET
@PRESTO_LABEL="presto-0.234";
SET
@IO_FILE_LABEL="io_file-1.0";
SET
@OPENLOOKENG_LABEL="openlookeng-1.5.0";
-- 衍生变量：
SET
@SPARK_ALL=CONCAT(''*-*,'',@SPARK_LABEL);
SET
@SPARK_IDE=CONCAT(''*-IDE,'',@SPARK_LABEL);
SET
@SPARK_NODE=CONCAT(''*-nodeexecution,'',@SPARK_LABEL);
SET
@SPARK_VISUALIS=CONCAT(''*-Visualis,'',@SPARK_LABEL);

SET
@HIVE_ALL=CONCAT(''*-*,'',@HIVE_LABEL);
SET
@HIVE_IDE=CONCAT(''*-IDE,'',@HIVE_LABEL);
SET
@HIVE_NODE=CONCAT(''*-nodeexecution,'',@HIVE_LABEL);

SET
@PYTHON_ALL=CONCAT(''*-*,'',@PYTHON_LABEL);
SET
@PYTHON_IDE=CONCAT(''*-IDE,'',@PYTHON_LABEL);
SET
@PYTHON_NODE=CONCAT(''*-nodeexecution,'',@PYTHON_LABEL);

SET
@PIPELINE_ALL=CONCAT(''*-*,'',@PIPELINE_LABEL);
SET
@PIPELINE_IDE=CONCAT(''*-IDE,'',@PIPELINE_LABEL);

SET
@JDBC_ALL=CONCAT(''*-*,'',@JDBC_LABEL);
SET
@JDBC_IDE=CONCAT(''*-IDE,'',@JDBC_LABEL);

SET
@PRESTO_ALL=CONCAT(''*-*,'',@PRESTO_LABEL);
SET
@PRESTO_IDE=CONCAT(''*-IDE,'',@PRESTO_LABEL);

SET
@IO_FILE_ALL=CONCAT(''*-*,'',@IO_FILE_LABEL);
SET
@IO_FILE_IDE=CONCAT(''*-IDE,'',@IO_FILE_LABEL);

SET
@OPENLOOKENG_ALL=CONCAT(''*-*,'',@OPENLOOKENG_LABEL);
SET
@OPENLOOKENG_IDE=CONCAT(''*-IDE,'',@OPENLOOKENG_LABEL);

-- Global Settings
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.yarnqueue'', ''yarn队列名'', ''yarn队列名'', ''ide'', ''None'', NULL, ''0 '', ''0 '', ''1 '',
        ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.yarnqueue.instance.max'', ''取值范围：1-128，单位：个'', ''队列实例最大个数'', ''30 '', ''Regex'',
        ''^(?:[1-9]\\d?|[1234]\\d{2}|128)$'', ''0 '', ''0 '', ''1 '', ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.yarnqueue.cores.max'', ''取值范围：1-500，单位：个'', ''队列CPU使用上限'', ''150 '', ''Regex'',
        ''^(?:[1-9]\\d?|[1234]\\d{2}|500)$'', ''0 '', ''0 '', ''1 '', ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.yarnqueue.memory.max'', ''取值范围：1-1000，单位：G'', ''队列内存使用上限'', ''300G'', ''Regex'',
        ''^([1-9]\\d{0,2}|1000)(G|g)$'', ''0 '', ''0 '', ''1 '', ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.client.memory.max'', ''取值范围：1-100，单位：G'', ''全局各个引擎内存使用上限'', ''20G'', ''Regex'',
        ''^([1-9]\\d{0,1}|100)(G|g)$'', ''0 '', ''0 '', ''1 '', ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.client.core.max'', ''取值范围：1-128，单位：个'', ''全局各个引擎核心个数上限'', ''10 '', ''Regex'',
        ''^(?:[1-9]\\d?|[1][0-2][0-8])$'', ''0 '', ''0 '', ''1 '', ''队列资源'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`)
VALUES (''wds.linkis.rm.instance'', ''范围：1-20，单位：个'', ''全局各个引擎最大并发数'', ''10 '', ''NumInterval'', ''[1, 20]'', ''0 '',
        ''0 '', ''1 '', ''队列资源'');
-- spark
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''wds.linkis.rm.instance'', ''范围：1-20，单位：个'', ''spark引擎最大并发数'', ''10 '', ''NumInterval'', ''[1, 20]'', ''0 '',
        ''0 '', ''1 '', ''队列资源'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''spark.executor.instances'', ''取值范围：1-40，单位：个'', ''spark执行器实例最大并发数'', ''1 '', ''NumInterval'', ''[1, 40]'',
        ''0 '', ''0 '', ''2 '', ''spark资源设置'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''spark.executor.cores'', ''取值范围：1-8，单位：个'', ''spark执行器核心个数'', ''1 '', ''NumInterval'', ''[1, 2]'', ''0 '',
        ''0 '', ''1 '', ''spark资源设置'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''spark.executor.memory'', ''取值范围：1-15，单位：G'', ''spark执行器内存大小'', ''1g'', ''Regex'', ''^([1-9]|1[0-5])(G|g)$'',
        ''0 '', ''0 '', ''3 '', ''spark资源设置'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''spark.driver.cores'', ''取值范围：只能取1，单位：个'', ''spark驱动器核心个数'', ''1 '', ''NumInterval'', ''[1, 1]'', ''0 '',
        ''1 '', ''1 '', ''spark资源设置'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''spark.driver.memory'', ''取值范围：1-15，单位：G'', ''spark驱动器内存大小'', ''1g'', ''Regex'', ''^([1-9]|1[0-5])(G|g)$'',
        ''0 '', ''0 '', ''1 '', ''spark资源设置'', ''spark'');
INSERT INTO `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`,
                                                  `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`,
                                                  `engine_conn_type`)
VALUES (''wds.linkis.engineconn.max.free.time'', ''取值范围：3m, 15m, 30m, 1h, 2h '', ''引擎空闲退出时间'', ''1h'', ''OFT'', ''[\"1h\",\"2h\",\"30m\",\"15m\",\"3m\"]'', ''0'', ''0'', ''1'', ''spark引擎设置'', ''spark'');
...
INSERT INTO `linkis_ps_dm_datasource_type_key` (`data_source_type_id`, `key`, `name`, `default_value`, `value_type`, `scope`, `require`, `description`, `value_regex`, `ref_id`, `ref_value`, `data_source`, `update_time`, `create_time`) VALUES (1, ''driverClassName'', ''连接驱动类名'', ''com.mysql.jdbc.Driver'', ''TEXT'', NULL, 1, ''mysql driver class name'', NULL, NULL, NULL, NULL,  now(), now());
...
INSERT INTO `linkis_ps_dm_datasource_type_key` (`data_source_type_id`, `key`, `name`, `default_value`, `value_type`, `scope`, `require`, `description`, `value_regex`, `ref_id`, `ref_value`, `data_source`, `update_time`, `create_time`) VALUES (1, ''databaseName'', ''数据库名'', NULL, ''TEXT'', NULL, 0, ''数据库名'', NULL, NULL, NULL, NULL,  now(), now());
