...
|linkis-engineconn-plugin-core |20001|The dist of EngineConn is empty,engineConnType(EngineConn 的 dist 为空,engineConnType):|DIST_IS_EMPTY|EngineconnCoreErrorCodeSummary|
|linkis-engineconn-plugin-core |20001|The dist of EngineConn is irregular, both the version dir and non-version dir are exist,engineConnType(EngineConn的dist是不规则的，版本目录和非版本目录都存在,engineConnType):|DIST_IRREGULAR_EXIST|EngineconnCoreErrorCodeSummary|
|linkis-engineconn-plugin-core |20001|System have no permission to delete old engineConn file,File(系统无权删除旧的engineConn文件,File):|NO_PERMISSION_FILE|EngineconnCoreErrorCodeSummary|
