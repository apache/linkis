...
class AsynRPCMessageBus(capacity: Int, busName: String)
  ...
  override protected val dropEvent: DropEvent = new DropEvent {
    override def onDropEvent(event: RPCMessageEvent): Unit = throw new DWCRPCRetryException("Async RPC Consumer Queue is full, please retry after some times.")
    override def onBusStopped(event: RPCMessageEvent): Unit = throw new RPCInitFailedException(RPCErrorConstants.RPC_INIT_ERROR, "Async RPC Consumer Thread has stopped!")
  }
}
