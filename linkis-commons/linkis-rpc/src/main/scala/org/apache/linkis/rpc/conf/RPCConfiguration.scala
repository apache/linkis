...
object RPCConfiguration {
  ...
  val PUBLIC_SERVICE_LIST: Array[String] = CommonVars(
    ...
    "cs,contextservice,data-source-manager,metadataquery,metadatamanager,query,jobhistory,application,configuration,filesystem,udf,variable,microservice,errorcode,bml,datasource,basedata_manager"
  ).getValue.split(",")
