...
class FIFOSchedulerTest {
  ...
  var fIFOScheduler: FIFOScheduler = _

  @BeforeEach
  def prepareScheduler: Unit = {
    schedulerContext = new FIFOSchedulerContextImpl(100)
    fIFOScheduler = new FIFOScheduler(schedulerContext)
  }

  @Test
  def testFIFOSchedulerInit: Unit = {
    fIFOScheduler.init()
    assertEquals("FIFOScheduler", fIFOScheduler.getName)
  }

  @Test
  def testFIFOSchedulerSubmit: Unit = {
    val userJob = new UserJob()
