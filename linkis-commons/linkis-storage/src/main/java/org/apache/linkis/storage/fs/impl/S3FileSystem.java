...
public class S3FileSystem extends FileSystem {
    ...
    public OutputStream write(FsPath dest, boolean overwrite) throws IOException {
        try (InputStream inputStream = read(dest); OutputStream outputStream = new S3OutputStream(s3Client, bucket, dest.getPath())) {
            if (overwrite) {
    ...
    public List<FsPath> list(FsPath path) throws IOException {
        try {
            if (!StringUtils.isEmpty(path.getPath())) {
                ...
                return s3ObjectSummaries.stream().filter(summary -> !isInitFile(summary)).map(summary -> {
