...
package com.apache.wedatasphere.linkis.storage.domain
...
/**
  *
  * @param id Engine unique Id(engine唯一的Id)
  * @param fsType Fs type(fs类型)
  * @param creatorUser Create a user to start the corresponding jvm user(创建用户为对应启动的jvm用户)
  * @param proxyUser Proxy user(代理用户)
  * @param clientIp client Ip for whitelist control(ip用于白名单控制)
  * @param methodName Method name called(调用的方法名)
  * @param params  Method parameter(方法参数)
  */
...
object MethodEntitySerializer{
  ...
  /**
    * Serialized to code as a MethodEntity object
    * 序列化为code为MethodEntity对象
    * @param code
    * @return
    */
  ...
  /**
    * Serialize a java object as a string
    * 序列化java对象为字符串
    * @param value
    * @return
    */
  ...
  /**
    * Deserialize a string into a java object
    * 将字符串解序列化为java对象
    * @param json
    * @param classType
    * @tparam T
