...
class StorageMultiExcelWriter(override val outputStream: OutputStream)
  ...
  override def init = {
    ...
    //1.Make table adaptive to column width（让表自适应列宽）
    if (sheet != null) {
      sheet.trackAllColumnsForAutoSizing()
      0 to columnCounter foreach (sheet.autoSizeColumn)
    }
    //2.reset parameters（重置参数）
    //2.1 columnCounter 归0
    columnCounter = 0
    //2.2 Create a new sheet （创建新sheet）
    sheet = workBook.createSheet(s"resultset$sheetIndex")
    //2.3 Sheetindex auto increment （sheetIndex自增）
    sheetIndex += 1
    //2.4 Set types to null （types 置空）
    types = null
    //2.5 Rowpoint to 0 rowPoint， number of record lines (归0 记录行数)
    rowPoint = 0
    //2.6 Styles empty (styles 清空)
    styles.clear()
  }
}
