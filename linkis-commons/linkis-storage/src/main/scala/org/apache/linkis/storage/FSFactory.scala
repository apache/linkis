...
object FSFactory extends Logging {
   ...
   *      obtained is the proxy to the Remote (shared storage machine root) FS 3. If it is HDFS,
   *      it returns the FS of the process user. 1、如果这台机器装有共享存储则这里获得的file://类型的FS为该进程用户的FS
  ...
  /**
   *   1. If the process user is passed and the proxy user and the process user are consistent,
   *      the file:// type FS is the FS of the process user (the shared storage exists) 2, if the
   *      process user is passed and the proxy user and the process user are consistent and there
   *      is no shared storage, the file:// type FS is the proxy to the remote (shared storage
   *      machine root) FS 3. If the passed proxy user and process user are consistent, the hdfs
   *      type is the FS of the process user. 4. If the proxy user and the process user are
   *      inconsistent, the hdfs type is the FS after the proxy.
