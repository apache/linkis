...
public class LinkisClientApplication {
        ...
        if (StringUtils.isBlank(configPath)) {
            throw new PropsException(
                    ...
                    "configuration root path specified by env variable: " +
                            AppKeys.CLIENT_CONFIG_ROOT_KEY + " is empty.");
        }
        ...
        for (ClientProperties properties : loaderResult) {
            if (StringUtils.equals(properties.getPropsId(), AppKeys.LINKIS_CLIENT_USER_CONFIG)) {
                for (Map.Entry prop : properties.entrySet()) {
                    if (StringUtils.startsWith((String) prop.getKey(), AppKeys.LINKIS_CLIENT_NONCUSTOMIZABLE)) {
     ...
     */
        JobOperatorBuilder builder = new LinkisOperatorBuilder()
        ...
        return new ProcessedData(AppConstants.DUMMY_CID,
                params.getCmdType(),
                stdVarAccess,
                sysVarAccess
        );
    ...
    private static FinishedData exec(ProcessedData data) throws Exception {
        ...
        ExecutionSuite suite = ExecutionSuiteFactory.getSuite(data.getCmdType(), data.getStdVarAccess(), data.getSysVarAccess());
