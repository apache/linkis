...
/**
 * @description: UjesClientDriver encapsulates UjesClient directly, calling apis to submit job/
 *     query info etc, and translating UjesClient's exception.
 */
public class UjesClientDriver implements LinkisClientDriver {
    ...
    public void initDriver(LinkisClientDriverContext context) throws LinkisClientRuntimeException {
        if (null == context) {
            throw new ExecutorException(
                    ...
                    "Cannot init UjesClientDriver: driver context is null");
        }
        if (!(context instanceof UjesClientDriverContext)) {
            throw new ExecutorException(
                    ...
                    ErrorLevel.ERROR,
                    CommonErrMsg.ExecutionInitErr,
                    "Cannot init UjesClientDriver: driver context is not instance of UjesClientDriverContext");
        }
            ...
            try {
                ...
                if (StringUtils.isBlank(ujesContext.getAuthenticationStrategyStr())
                        || !LinkisConstants.AUTH_STRATEGY_TOKEN.equalsIgnoreCase(
                                ujesContext.getAuthenticationStrategyStr())) {
                    authenticationStrategy =
                            new StaticAuthenticationStrategy(); // this has to be newed here
                    // otherwise log-in fails for static
                } else {
                ...
                DWSClientConfig config =
                        ((DWSClientConfigBuilder)
                                        (builder.addServerUrl(ujesContext.getGatewayUrl())
                                                ...
                                                .maxConnectionSize(5)
                                                .retryEnabled(false)
                                                .readTimeout(ujesContext.getReadTimeoutMills())
                                                .setAuthenticationStrategy(
                                                        authenticationStrategy) // token/static
                                                .setAuthTokenKey(
                                                        ujesContext.getTokenKey()) // static
                                                // authentication ->
                                                // submitUser must
                                                // be the same as
                                                // tokenKey
                                                .setAuthTokenValue(ujesContext.getTokenValue())))
                                .setDWSVersion(ujesContext.getDwsVersion())
                                .build();
    ...
    public JobSubmitResult submit(LinkisJob linkisJob) throws LinkisClientRuntimeException {
        ...
        if (StringUtils.isBlank(jobSubmitResult.getTaskID())) {
            throw new ExecutorException(
                    ...
                    ErrorLevel.ERROR,
                    CommonErrMsg.ExecutionErr,
                    "Fail to get TaskId from Linkis after job submission");
        }
     ...
     */
    @Override
    public JobInfoResult queryJobInfo(String user, String taskID)
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                String msg =
                        MessageFormat.format(
                                "Get job info failed. retry time : {0}/{1}",
        ...
        if (jobInfoResult == null || 0 != jobInfoResult.getStatus()) {
            ...
            String msg =
                    MessageFormat.format(
                            "Get info failed. Retry exhausted. taskID={0}, Reason: {1}",
                            taskID, reason);
            throw new ExecutorException(
                    "EXE0013", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
    ...
    @Override
    public JobLogResult queryRunTimeLogFromLine(
            String user, String taskID, String execID, int fromLine)
            throws LinkisClientRuntimeException {
        checkInit();
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                String msg =
                        MessageFormat.format(
                                "Get log failed. Retry time : {0}/{1}", retryTime, MAX_RETRY_TIME);
                //                logger.warn("", new ExecutorException("EXE0016", ErrorLevel.ERROR,
                // CommonErrMsg.ExecutionErr, msg));
                if (retryTime >= MAX_RETRY_TIME) {
                    throw new ExecutorException(
        ...
        if (logResult == null || 0 != logResult.getStatus()) {
            ...
            String msg =
                    MessageFormat.format(
                            "Get log failed. Retry exhausted. taskID={0}, Reason: {1}",
                            taskID, reason);
            //            logger.warn("", new ExecutorException("EXE0016", ErrorLevel.ERROR,
            // CommonErrMsg.ExecutionErr, msg));
            throw new ExecutorException(
    ...
    @Override
    public OpenLogResult queryPersistedLogAll(String logPath, String user, String taskID)
            throws LinkisClientRuntimeException {
        checkInit();
        int retryCnt = 0;
            ...
            try {
                ...
                if (openLogResult == null
                        ...
                        || StringUtils.isBlank(
                                openLogResult
                                        .getLog()[
                                        UjesClientDriverConstants.IDX_FOR_LOG_TYPE_ALL])) {
                    String reason;
                    if (openLogResult == null) {
                ...
                String msg =
                        MessageFormat.format(
                                "Get log from openLog failed. retry time : {0}/{1}",
                                retryCnt, MAX_RETRY_TIMES);
                if (e instanceof LinkisException) {
                    msg += " " + e.toString();
                }
                logger.warn(msg, e);
    ...
    @Override
    public JobProgressResult queryProgress(String user, String taskID, String execId)
            throws LinkisClientRuntimeException {
        checkInit();
        JobExecuteResult executeResult = new JobExecuteResult();
        executeResult.setTaskID(taskID);
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                String msg =
                        MessageFormat.format(
                                "Get progress failed. retry time : {0}/{1}",
                                retryTime, MAX_RETRY_TIME);
                if (e instanceof LinkisException) {
                    msg += " " + e.toString();
                }
        ...
        if (jobProgressResult == null || 0 != jobProgressResult.getStatus()) {
            ...
            String msg =
                    MessageFormat.format(
                            "Get progress failed. Retry exhausted. taskID={0}, Reason: {1}",
                            taskID, reason);
            throw new ExecutorException(
    ...
    @Override
    public ResultSetResult[] queryAllResults(String user, String taskID, String resultSetLocation)
            throws LinkisClientRuntimeException {
    ...
    public String[] queryResultSetPaths(String user, String taskID, String resultLocation) {
        ...
        if (null == jobInfoResult) {
            ...
            logger.error(msg);
            throw new ExecutorException(
                    JobStatus.UNKNOWN, "EXE0021", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
        ...
        if (StringUtils.isBlank(jobInfoResult.getRequestPersistTask().getResultLocation())) {
            logger.warn(
                    "Cannot get resultLocation from linkis server. May not be able to display resultSet");
        }
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                resultSetArray =
                        jobInfoResult.getResultSetList(client); // this makes call to server
                ...
                String msg =
                        MessageFormat.format(
                                "Get resultSetArray failed. retry time : {0}/{1}",
                                retryTime, MAX_RETRY_TIME);
                if (e instanceof LinkisException) {
                    msg += " " + e.toString();
                }
    ...
    @Override
    public ResultSetResult queryResultSetGivenResultSetPath(
            String resultSetPath, String user, Integer page, Integer pageSize) {
        checkInit();
        int retryTime = 0;
        final int MAX_RETRY_TIME = UjesClientDriverConstants.DRIVER_REQUEST_MAX_RETRY_TIME;
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                String msg =
                        MessageFormat.format(
                                "Get resultSet failed. retry time : {0}/{1}",
                                retryTime, MAX_RETRY_TIME);
                if (e instanceof LinkisException) {
                    msg += " " + e.toString();
                }
    ...
    @Override
    public JobKillResult kill(String user, String taskId, String execId)
            throws LinkisClientRuntimeException {
        checkInit();
        ...
        if (result == null || 0 != result.getStatus()) {
            ...
            String msg =
                    MessageFormat.format(
                            "Kill job failed. Retry exhausted. taskId={0}, Reason: {1}",
                            taskId, reason);
            throw new ExecutorException(
