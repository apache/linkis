...
public class UjesClientDriverBuilder extends BuildableByVarAccess<UjesClientDriver> {
    ...
    public UjesClientDriver build() {
        String gatewayUrl =
                stdVarAccess.getVar(String.class, LinkisClientKeys.LINKIS_COMMON_GATEWAY_URL);
        ...
        String authKey =
                stdVarAccess.getVar(String.class, LinkisClientKeys.LINKIS_COMMON_TOKEN_KEY);
        String authValue =
                stdVarAccess.getVar(String.class, LinkisClientKeys.LINKIS_COMMON_TOKEN_VALUE);

        String authenticationStrategy =
                stdVarAccess.getVarOrDefault(
                        String.class,
                        LinkisClientKeys.LINKIS_COMMON_AUTHENTICATION_STRATEGY,
                        LinkisConstants.AUTH_STRATEGY_STATIC);

        long connectionTimeout =
                stdVarAccess.getVarOrDefault(
                        Long.class, LinkisClientKeys.UJESCLIENT_COMMON_CONNECTT_TIMEOUT, 30000L);
        boolean discoveryEnabled =
                stdVarAccess.getVarOrDefault(
                        Boolean.class, LinkisClientKeys.UJESCLIENT_COMMON_DISCOVERY_ENABLED, false);
        boolean loadbalancerEnabled =
                stdVarAccess.getVarOrDefault(
                        Boolean.class,
                        LinkisClientKeys.UJESCLIENT_COMMON_LOADBALANCER_ENABLED,
                        true);
        int maxConnectionSize =
                stdVarAccess.getVarOrDefault(
                        Integer.class, LinkisClientKeys.UJESCLIENT_COMMON_MAX_CONNECTION_SIZE, 5);
        boolean retryEnabled =
                stdVarAccess.getVarOrDefault(
                        Boolean.class, LinkisClientKeys.UJESCLIENT_COMMON_RETRY_ENABLED, false);
        long readTimeout =
                stdVarAccess.getVarOrDefault(
                        Long.class, LinkisClientKeys.UJESCLIENT_COMMON_READTIMEOUT, 30000L);
        String dwsVersion =
                stdVarAccess.getVarOrDefault(
                        String.class, LinkisClientKeys.UJESCLIENT_COMMON_DWS_VERSION, "v1");
