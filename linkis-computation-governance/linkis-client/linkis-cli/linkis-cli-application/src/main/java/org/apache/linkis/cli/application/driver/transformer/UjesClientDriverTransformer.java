...
/**
 * @description: convert data in order to fit response of {@link UjesClientDriver} into a form that
 *     acceptable by linkis-cli
 */
public class UjesClientDriverTransformer implements DriverTransformer {
    ...
    public JobStatus convertJobStatus(String jobStatus) {
        if (StringUtils.isNotBlank(jobStatus)) {
            ...
            else if (JobStatus.WAIT_FOR_RETRY.name().equalsIgnoreCase(jobStatus))
                return JobStatus.WAIT_FOR_RETRY;
            else if (JobStatus.SCHEDULED.name().equalsIgnoreCase(jobStatus))
            ...
            else if (JobStatus.CANCELLED.name().equalsIgnoreCase(jobStatus))
    ...
    public JobExec convertAndUpdateExecData(JobExec execData, DWSResult result) {
        ...
        if (execData instanceof LinkisJobSubmitExec) {
            ...
            if (result instanceof JobInfoResult) {
                ...
                execData =
                        updateExecDataByJobSubmitResult(linkisExecData, (JobSubmitResult) result);
            } else {
                throw new TransformerException(
                        "TFM0002",
                        ErrorLevel.ERROR,
                        CommonErrMsg.TransformerException,
                        "Failed to convert DWSResult into JobSubmitExec: \""
    ...
    public JobExecModel convertAndUpdateModel(JobExecModel model, DWSResult result) {
        ...
        if (result instanceof JobLogResult && model instanceof LinkisJobIncLogModel) {
            model =
                    updateIncLogModelByJobLogResult(
                            (LinkisJobIncLogModel) model, (JobLogResult) result);
            ...
            model =
                    updateIncLogModelByOpenLogResult(
                            (LinkisJobIncLogModel) model, (OpenLogResult) result);
    ...
    private JobSubmitExec updateExecDataByJobSubmitResult(
            LinkisJobSubmitExec execData, JobSubmitResult jobExecuteResult) {

        if (jobExecuteResult != null && jobExecuteResult.getTaskID() != null) {
    ...
    private LinkisJobSubmitExec updateSubmitExecByJobInfoResult(
            LinkisJobSubmitExec execData, JobInfoResult result) {
        ...
        if (result != null && StringUtils.isNotBlank(result.getUser())) {
        ...
        if (result != null
                ...
                && StringUtils.isNotBlank(
                        (String) result.getTask().get(LinkisKeys.KEY_ERROR_DESC))) {
    ...
    private LinkisJobInfo updateInfoDataByJobInfoResult(
            LinkisJobInfo execData, JobInfoResult result) {
        if (result != null
                && result.getRequestPersistTask() != null
                && result.getRequestPersistTask().getTaskID() != null) {
            String candidateTaskID = String.valueOf(result.getRequestPersistTask().getTaskID());
        ...
        if (result != null && result.getRequestPersistTask() != null) {
            ...
            execData.setRequestApplicationName(
                    result.getRequestPersistTask().getRequestApplicationName());
            execData.setExecuteApplicationName(
                    result.getRequestPersistTask().getExecuteApplicationName());
        }
    ...
    private LinkisJobKill updateKillDataByJobInfoResult(
            LinkisJobKill killData, JobInfoResult result) {
    ...
    private LinkisJobIncLogModel updateIncLogModelByJobLogResult(
            LinkisJobIncLogModel model, JobLogResult jobLogResult) {
        ...
        if (jobLogResult != null
                ...
                && StringUtils.isNotBlank(
                        jobLogResult
                                .getLog()
                                .get(UjesClientDriverConstants.IDX_FOR_LOG_TYPE_ALL))) {
            model.setFromLine(jobLogResult.getFromLine());
            String incLog =
                    jobLogResult.getLog().get(UjesClientDriverConstants.IDX_FOR_LOG_TYPE_ALL);
            model.writeIncLog(incLog);
        }
        return model;
    }

    private LinkisJobIncLogModel updateIncLogModelByOpenLogResult(
            LinkisJobIncLogModel model, OpenLogResult openLogResult) {
        if (openLogResult != null
                && openLogResult.getLog() != null
                && StringUtils.isNotBlank(
                        openLogResult.getLog()[UjesClientDriverConstants.IDX_FOR_LOG_TYPE_ALL])) {
    ...
    private LinkisJobResultModel updateModelByResultSet(
            LinkisJobResultModel model, ResultSetResult resultSetResult) {
        ...
        if (resultSetResult != null) {
            ...
            resultModel.setResultContent(
                    resultSetResult.getFileContent()); // can be null if reaches non-existing page
    ...
    private LinkisJobIncLogModel updateModelByJobInfo(
            LinkisJobIncLogModel model, JobInfoResult result) {
