...
public class LinkisJobOperator implements JobOperator {
    ...
    public void checkInit() throws LinkisClientRuntimeException {
        if (client == null) {
            throw new LinkisClientExecutionException("EXE0011", ErrorLevel.ERROR, CommonErrMsg.ExecutionInitErr, "UjesClientDriver is null");
        }
    }
     ...
     * @return
     */
    public LinkisOperResultAdapter submit(LinkisSubmitDesc jobDesc) throws LinkisClientRuntimeException {
        checkInit();
...
//      jobExecuteResult = client.execute(jobExecuteAction);
            ...
            throw new LinkisClientExecutionException("EXE0011", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, "Failed to submit job", e);
        }

        if (jobSubmitResult == null || 0 != jobSubmitResult.getStatus() || StringUtils.isBlank(jobSubmitResult.getTaskID())) {
            String reason;
            ...
            throw new LinkisClientExecutionException("EXE0012", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                if (logResult == null || 0 != logResult.getStatus()) {
                    ...
                    logger.debug("", new LinkisClientExecutionException("EXE0015", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg));
                } else {
                    break;
                }
            } catch (Exception e) {
                String msg = MessageFormat.format("Get log failed. Retry time : {0}/{1}", retryTime, MAX_RETRY_TIME);
//                logger.warn("", new LinkisClientExecutionException("EXE0016", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg));
                if (retryTime >= MAX_RETRY_TIME) {
                    throw new LinkisClientExecutionException("EXE0016", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg, e);
                }
            }
            Utils.doSleepQuietly(UJESConstants.DRIVER_QUERY_SLEEP_MILLS);
        }
...
//            logger.warn("", new LinkisClientExecutionException("EXE0016", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg));
            throw new LinkisClientExecutionException("EXE0016", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
    ...
    public LinkisOperResultAdapter queryPersistedLogFromLine(String logPath, String user, String taskID, int fromLine) {
        return new UJESResultAdapter(new OpenLogResult2(queryPersistedLogInternal(logPath, user, taskID), fromLine));
    }
    ...
    private OpenLogResult queryPersistedLogInternal(String logPath, String user, String taskID) throws LinkisClientRuntimeException {
        ...
        while (retryCnt++ < MAX_RETRY_TIMES) {
            try {
                ...
                if (retryCnt >= MAX_RETRY_TIMES) {
                    throw new LinkisClientExecutionException("EXE0017", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, "Get log from openLog failed. Retry exhausted. taskID=" + taskID, e);
                }
            }
            Utils.doSleepQuietly(UJESConstants.DRIVER_QUERY_SLEEP_MILLS);
        }
        if (openLogResult == null ||
            ...
            if (retryCnt >= MAX_RETRY_TIMES) {
                ...
                throw new LinkisClientExecutionException("EXE0017", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
            }
        }
    ...
    public UJESResultAdapter queryProgress(String user, String taskID, String execId) throws LinkisClientRuntimeException {
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                logger.warn(msg, e);
                if (retryTime >= MAX_RETRY_TIME) {
                    throw new LinkisClientExecutionException("EXE0019", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, "Get progress failed. Retry exhausted. taskID=" + taskID, e);
                }
            }
            Utils.doSleepQuietly(UJESConstants.DRIVER_QUERY_SLEEP_MILLS);
        }
        ...
        if (jobProgressResult == null || 0 != jobProgressResult.getStatus()) {
            ...
            throw new LinkisClientExecutionException("EXE0020", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
    ...
    public LinkisOperResultAdapter queryResultSetPaths(String user, String taskID, String resultLocation) {
        ...
        if (resultSetArray == null || 0 == resultSetArray.length) {
            ...
            logger.warn(msg);
            throw new LinkisClientExecutionException("EXE0023", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg + ". taskID=" + taskID);
        }
    ...
    public LinkisOperResultAdapter kill(String user, String taskId, String execId) throws LinkisClientRuntimeException {
        ...
        while (retryTime++ < MAX_RETRY_TIME) {
            try {
                ...
                logger.warn(msg, e);
                if (retryTime >= MAX_RETRY_TIME) {
                    throw new LinkisClientExecutionException("EXE0025", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, "Kill job failed. taskId={0} Retry exhausted.", taskId, e);
                }
            }
            Utils.doSleepQuietly(UJESConstants.DRIVER_QUERY_SLEEP_MILLS);
        }
        if (result == null || 0 != result.getStatus()) {
            ...
            throw new LinkisClientExecutionException("EXE0025", ErrorLevel.ERROR, CommonErrMsg.ExecutionErr, msg);
        }
