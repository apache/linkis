...
public class LinkisResultInfoPresenter implements Presenter {
    ...
    public void present(Model model, PresentWay presentWay) {
        if (!(model instanceof LinkisResultInfoModel)) {
            throw new PresenterException("PST0001", ErrorLevel.ERROR, CommonErrMsg.PresenterErr, "Input model for \"LinkisResultInfoPresenter\" is not instance of \"LinkisResultInfoModel\"");
        }
    ...
    protected String formatResultIndicator(LinkisResultInfoModel model) {
        ...
        if (model.getJobStatus().isJobSuccess()) {
            ...
            infoBuilder.append("============Result:================").append(System.lineSeparator())
                    .append("TaskId:").append(model.getJobID()).append(System.lineSeparator())
