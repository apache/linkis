...
public class LinkisJobResultPresenter extends QueryBasedPresenter {
    ...
    public void present(PresenterModel model) {
        ...
        if (!(model instanceof LinkisJobResultModel)) {
            throw new PresenterException(
                    ...
                    "Input model for \"LinkisJobResultPresenter\" is not instance of \"LinkisJobResultModel\"");
        }
        ...
        if (!((LinkisJobResultModel) model).isJobSuccess()) {
            logger.info(
                    "Job status is not success but \'"
                            + ((LinkisJobResultModel) model).getJobStatus()
        ...
        if (resultSetPaths == null) {
            Exception e =
                    new PresenterException(
                            "PST0012",
                            ErrorLevel.ERROR,
                            CommonErrMsg.PresenterErr,
                            "ResultPresenter got null as ResultSet");
            logger.warn("", e);
            return;
        }
        ...
        for (int i = 0; i < resultSetPaths.length; i++) {
            ...
            while (true) {
                ...
                try {
                    result =
                            clientDriver.queryResultSetGivenResultSetPath(
                                    resultSetPath,
                                    resultModel.getUser(),
                                    curPage,
                ...
                resultModel =
                        (LinkisJobResultModel)
                                transformer.convertAndUpdateModel(resultModel, result);
                if (curPage == 1) {
                    try {
                        ...
                        logger.warn(
                                "Cannot convert ResultSet-Meta-Data. ResultSet-Meta-Data:"
                                        + JacksonUtils.toJsonFormat(
                                                resultModel.getResultMetaData()),
                                e);
                        continue;
                    }
                }
                ...
                if (curPage == 1) {
                    ...
                    displayDriver.doOutput(
                            new FileOutData(outputPath, fileName, resultSb.toString(), true));
                } else {
                    String contentStr = formatResultContent(false, metaData, contentData);
                    resultSb.append(contentStr);
                    displayDriver.doOutput(
                            new FileOutData(outputPath, fileName, resultSb.toString(), false));
                }
        ...
        if (((LinkisJobResultModel) model).getOutputWay() == OutputWay.TEXT_FILE
            ...
            LogUtils.getInformationLogger()
                    .info("ResultSet has been successfully written to path: " + outputPath);
        }
    }
    ...
    protected String formatResultIndicator(LinkisJobResultModel model) {
        ...
        if (model.isJobSuccess()) {
            ...
            if (model.getErrCode() != null) {
                infoBuilder
                        .append("errCode: ")
                        .append(model.getErrCode())
                        .append(System.lineSeparator());
            }
            if (StringUtils.isNotBlank(model.getErrDesc())) {
                infoBuilder
                        .append("errDesc: ")
                        .append(model.getErrDesc())
                        .append(System.lineSeparator());
            }
        } else {
            throw new PresenterException(
    ...
    protected String formatResultContent(
            ...
            List<LinkedHashMap<String, String>> metaData,
            List<List<String>> contentData) {

        StringBuilder outputBuilder = new StringBuilder();
        if (contentData == null || contentData.size() == 0) { // finished
            return outputBuilder
                    .append(AppConstants.RESULTSET_END_LOGO)
                    .append(System.lineSeparator())
                    .toString();
        }
        ...
        if (metaData != null && showMetaInfo) {
            for (LinkedHashMap<String, String> mapElement : metaData) {
                ...
                outputBuilder
                        .append(mapElement.entrySet().iterator().next().getValue())
                        .append("\t");
            }
            outputBuilder.append(System.lineSeparator());
