...
public abstract class AbstractFitter implements Fitter {
    ...
    /**
     * Parse arguments one by one. If an Option/Flag is not defined in 'commandTemplate', then
     * record it in 'remains' If all parameters defined in 'commandTemplate' are already set, then
     * any other Parameter is recorded in 'remains'
     *
    ...
    /**
     * If number of user input parameter is larger than what's defined in template, then set
     * parameter value based on input order and record the rests in 'remains'.
     *
