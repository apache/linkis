...
public abstract class AbstractFitter implements Fitter {
    ...
    protected CmdTemplate doFit(String[] args, CmdTemplate templateCopy, List<String> remains) throws LinkisClientRuntimeException {
        ...
        doFit(args, 0, templateCopy.getCmdType(), templateCopy.getOptionsMap(), templateCopy.getOptions(), parameters, remains);
     ...
     */
    private final void doFit(final String[] args, int start, CmdType cmdType, Map<String, CmdOption<?>> optionsMap, List<CmdOption<?>> options, List<CmdOption<?>> parameters, List<String> remains) throws LinkisClientRuntimeException {
        doFit(args, start, 0, cmdType, optionsMap, options, parameters, remains);
    }

    private final void doFit(final String[] args, final int argIdx, final int paraIdx, CmdType cmdType, Map<String, CmdOption<?>> optionsMap, List<CmdOption<?>> options, List<CmdOption<?>> parameters, List<String> remains)
            throws LinkisClientRuntimeException {
     ...
     * @throws LinkisClientRuntimeException when some argument is not configured
     */
    private final int setOptionValue(final String[] args, final int index, CmdType cmdType, Map<String, CmdOption<?>> optionsMap, List<String> remains) throws LinkisClientRuntimeException {
        int next = index + 1;
        ...
        if (optionsMap.containsKey(args[index])) {
            ...
            if (cmdOption instanceof Flag) {
                try {
                    ...
                    throw new CommandException("CMD0010", ErrorLevel.ERROR, CommonErrMsg.TemplateFitErr, cmdType, msg);
                }
                return next;
                ...
                if (next >= args.length || FitterUtils.isOption(args[next])) {
                    ...
                    throw new CommandException("CMD0011", ErrorLevel.ERROR, CommonErrMsg.TemplateFitErr, cmdType, msg);
                }
                try {
     ...
     * @throws LinkisClientRuntimeException
     */
    private final int setParameterValue(final String[] args, final int argIdx, final int paraIdx, List<CmdOption<?>> parameters, List<String> remains)
            throws LinkisClientRuntimeException {
