...
public class EngineConnLogOperator implements Operator {
    ...
    public Map<String, Object> apply(Map<String, Object> parameters) {
        ...
        String[] ignoreKeywordList = StringUtils.isNotEmpty(ignoreKeywords) ? ignoreKeywords.split(",") : new String[0];

        String onlyKeywords = getAs(parameters, "onlyKeywords", "");
        String[] onlyKeywordList = StringUtils.isNotEmpty(onlyKeywords) ? onlyKeywords.split(",") : new String[0];
        ...
        try {
            ...
            int ignoreLine = 0;
            Pattern linePattern = null != EngineConnLogOperator.MULTILINE_PATTERN.getValue() ? Pattern.compile(EngineConnLogOperator.MULTILINE_PATTERN.getValue()) : null;
    ...
    private String randomAndReversedReadLine(RandomAccessFile randomReader, ReversedLinesFileReader reversedReader) throws IOException {
    ...
    protected File getLogPath(Map<String, Object> parameters) {
        ...
        String engineConnLogDir = ECMConfiguration.ENGINECONN_ROOT_DIR() + File.separator + logDIrSuffix;
        ...
        logger.info(
                String.format("Try to fetch EngineConn(id: %s, instance: %s) logs from %s.", ticketId, engineConnInstance, logPath.getPath())
        );
        return logPath;
    }
    ...
    protected Triple<String, String, String> getEngineConnInfo(Map<String, Object> parameters){
        ...
        localDirsHandleService = DataWorkCloudApplication.getApplicationContext().getBean(LocalDirsHandleService.class);
        String engineConnLogDir = ECMConfiguration.ENGINECONN_ROOT_DIR() + File.separator + logDIrSuffix;
        String ticketId = getAs(parameters, "ticketId", "");
        String engineConnInstance = "";
    ...
    private boolean includeLine(String line, String[] onlyKeywordList, String[] ignoreKeywordList) {
        boolean accept = ignoreKeywordList.length == 0 || !Arrays.stream(ignoreKeywordList).anyMatch(line::contains);
        if (accept) {
            accept = onlyKeywordList.length == 0 || Arrays.stream(onlyKeywordList).anyMatch(line::contains);
        }
        return accept;
    }
}
