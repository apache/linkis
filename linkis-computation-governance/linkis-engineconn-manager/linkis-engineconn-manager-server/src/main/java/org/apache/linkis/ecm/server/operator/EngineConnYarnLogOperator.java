...
public class EngineConnYarnLogOperator extends EngineConnLogOperator {
    ...
    public File getLogPath(Map<String, Object> parameters) {
        ...
        if (!logPath.exists()) {
            String tempLogFile =
                    String.format(".yarn_%s_%d_%d", applicationId, System.currentTimeMillis(), Thread.currentThread().getId());
            try {
                String command = String.format("yarn logs -applicationId %s >> %s/%s", applicationId, rootLogDir, tempLogFile);
                ...
                if (waitFor && process.waitFor() == 0) {
                    command = String.format("mv %s/%s %s/yarn_%s", rootLogDir, tempLogFile, rootLogDir, applicationId);
                    ...
                    if (!logPath.exists()) {
                        throw new WarnException(
                                -1,
                                String.format("Fetch yarn logs timeout, log aggregation has not completed or is not enabled")
                        );
                    }
                }
            } catch (Exception e) {
                throw new WarnException(
                        -1,
                        String.format("Fail to fetch yarn logs application: %s, message: %s", applicationId, e.getMessage())
                );
            }
        }
        if (!logPath.exists() || !logPath.isFile()) {
            throw new WarnException(
                    ...
                    MessageFormat.format(EngineconnServerErrorCodeSummary.LOGFILE_IS_NOT_EXISTS.getErrorDesc(), logPath)
            );
        }
        logger.info(
                String.format("Try to fetch EngineConn(id: %s, instance: %s) yarn logs from %s in application id: %s", ticketId, engineConnInstance, logPath.getPath(), applicationId));
