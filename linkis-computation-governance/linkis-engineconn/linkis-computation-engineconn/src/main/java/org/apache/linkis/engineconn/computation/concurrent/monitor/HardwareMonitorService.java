...
public class HardwareMonitorService implements MonitorService {
    ...
    private static CommonVars<Double> MEMORY_MAX_USAGE = CommonVars.apply("linkis.engineconn.concurrent.max.memory.usage", 0.95);

    private static CommonVars<Double> CPU_MAX_USAGE = CommonVars.apply("linkis.engineconn.concurrent.max.cpu.usage", 0.99);
    ...
    public boolean isAvailable() {
        ...
        boolean isUnavailable = (memoryUsage > maxMemoryUsage && lastMemoryUsage > maxMemoryUsage) || (loadAverageUsage > maxCpuUsage && lastLoadAverageUsage > maxCpuUsage );
        if (isUnavailable) {
            LOG.warn("current load average {} is too high or memory usage {} is too high, over max cpu load avg={} and memory usage {}", loadAverageUsage, memoryUsage, maxCpuUsage, maxMemoryUsage);
        }
