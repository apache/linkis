...
class BmlEnginePreExecuteHook extends ComputationExecutorHook with Logging{
  ...
  private val bmlClient: BmlClient = if (StringUtils.isNotEmpty(processUser)) BmlClientFactory.createBmlClient(processUser) else BmlClientFactory.createBmlClient(defaultUser)
        ...
        case resources: util.List[Object] =>
          resources.asScala.foreach {
            case resource: util.Map[String, Object] => val fileName = resource.get(GovernanceConstant.TASK_RESOURCE_FILE_NAME_STR).toString
              ...
              val fullPath = if (workDir.endsWith(seperator)) pathType + workDir + fileName else pathType + workDir + seperator + fileName
