...
class CreatorRestLabelsHook extends ExecutorLabelsRestHook with Logging {
  ...
  override def restExecutorLabels(labels: Array[Label[_]]): Array[Label[_]] = {
    ...
    if (null != codeLabel && RunType.FUNCTION_MDQ_TYPE.toString.equalsIgnoreCase(codeLabel.getCodeType)) {
      logger.info(s"code Type Label($codeLabel) reset to scala")
      val newCodeLabel = LabelBuilderFactoryContext.getLabelBuilderFactory.createLabel(classOf[CodeLanguageLabel])
