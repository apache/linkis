...
public enum EngineconnCoreErrorCodeSummary implements LinkisErrorCode {
  ...
  CANNOT_HOME_PATH_EC(
      20001,
      "Could not find engineConn's home path, the path is:{0}(找不到 engineConn 的 home 路径，路径为：{0})"),
  CANNOT_HOME_PATH_DIST(
      20001,
      "Could not find the home path for engineconn dist at: {0}(找不到 engineconn dist 的 home 路径，路径为:{0})"),
  DIST_IS_EMPTY(
      20001,
