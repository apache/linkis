...
package com.apache.wedatasphere.linkis.entrance
...
import com.apache.wedatasphere.linkis.common.utils.Utils
import com.apache.wedatasphere.linkis.entrance.conf.EntranceConfiguration
import com.apache.wedatasphere.linkis.entrance.event._
import com.apache.wedatasphere.linkis.entrance.execute.EntranceJob
import com.apache.wedatasphere.linkis.entrance.job.EntranceExecutionJob
import com.apache.wedatasphere.linkis.entrance.log.LogReader
import com.apache.wedatasphere.linkis.entrance.restful.EntranceRestfulApi
import com.apache.wedatasphere.linkis.manager.label.constant.LabelKeyConstant
import com.apache.wedatasphere.linkis.manager.label.entity.engine.EngineTypeLabel
...
import com.apache.wedatasphere.linkis.protocol.utils.ZuulEntranceUtils
import com.apache.wedatasphere.linkis.rpc.Sender
import com.apache.wedatasphere.linkis.scheduler.queue.{Job, SchedulerEventState}
import com.apache.wedatasphere.linkis.server._
import com.apache.wedatasphere.linkis.server.conf.ServerConfiguration
import com.apache.wedatasphere.linkis.server.socket.controller.{ServerEvent, ServerEventService, SocketServerEvent}
...
class EntranceWebSocketService extends ServerEventService with EntranceEventListener with EntranceLogListener {
  ...
  Utils.defaultScheduler.scheduleAtFixedRate(new Runnable {
    override def run(): Unit = {
      ...
      val queues = entranceServer.getEntranceContext.getOrCreateScheduler().getSchedulerContext
        .getOrCreateConsumerManager.listConsumers().map(_.getConsumeQueue).toSet
  ...
  }, 60, 30, TimeUnit.SECONDS) //TODO Time interval(时间间隔做成参数)
  ...
  def dealExecute(event:ServerEvent):Message = {
    ...
    entranceServer.getEntranceContext.getOrCreateLogManager().onLogUpdate(job,
      LogUtils.generateInfo(s"You have submitted a new job at " + time))
    entranceServer.getEntranceContext.getOrCreateLogManager().onLogUpdate(job,
  ...
  def dealPause(event:ServerEvent, id:String) : Message = {
    var retMessage:Message = Message.error("Temporarily does not support the task pause(暂时不支持任务暂停)")
    retMessage.setStatus(1)
    retMessage.setMethod(restfulURI + "entrance/" + id + "/progress")
    retMessage
  }
  ...
  override def onEvent(event: EntranceEvent): Unit = event match {
    ...
    case EntranceProgressEvent(job, progress, progressInfo) => pushProgressToFrontend(job, progress, progressInfo)
    case _ =>
  }
