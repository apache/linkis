...
import org.apache.linkis.entrance.ecm.entity.Policy
...
object EngineConnManagerBuilder extends Logging {
  ...
  private val engineConnManagerClazzCache =
    new util.HashMap[String, Class[_ <: EngineConnManager]]()
          ...
          if (!ClassUtils.isInterfaceOrAbstract(clazz)) {
            ...
            if (engineConnManagerClazzCache.containsKey(manager.getPolicy().name())) {
              throw new RuntimeException(
                s"EngineConnManager Type cannot be duplicated ${manager.getPolicy()} "
              )
            }
            logger.warn(
              s"EngineConnManagerBuilder Instantiation: ${manager.getPolicy().name()}, clazz ${clazz}"
            )
