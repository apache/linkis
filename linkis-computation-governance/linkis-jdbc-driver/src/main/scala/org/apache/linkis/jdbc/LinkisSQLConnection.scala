...
class LinkisSQLConnection(private[jdbc] val ujesClient: UJESClient, props: Properties)
  ...
  private[jdbc] def getEngineType: EngineType = {
    ...
    if (props.containsKey(PARAMS)) {
      ...
      if (params != null & params.nonEmpty) {
        params.split(PARAM_SPLIT).map(_.split(KV_SPLIT)).foreach {
          case Array(k, v) if k.equals(LinkisDriver.ENGINE_TYPE) =>
            return EngineType.mapStringToEngineType(v)
          case _ =>
        }
      }
    }
  ...
  private[jdbc] def throwWhenClosed[T](op: => T): T =
    if (isClosed) throw new LinkisSQLException(LinkisErrorCode.CONNECTION_CLOSED)
    else op
  ...
  def removeStatement(statement: LinkisSQLStatement): Unit =
    runningSQLStatements.remove(statement)
  ...
  override def getMetaData: DatabaseMetaData = throwWhenClosed(
    new LinkisSQLDatabaseMetaData(this)
  )
  ...
  override def setReadOnly(readOnly: Boolean): Unit =
    throw new LinkisSQLException(
      LinkisErrorCode.NOSUPPORT_CONNECTION,
  ...
  override def rollback(): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "rollback not supported")

  override def nativeSQL(sql: String): String =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "nativeSQL not supported")
  ...
  override def getTypeMap: util.Map[String, Class[_]] =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "getTypeMap not supported")

  override def setTypeMap(map: util.Map[String, Class[_]]): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "setTypeMap not supported")
  ...
  override def setSavepoint(): Savepoint = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
    "setSavepoint not supported"
  )

  override def setSavepoint(name: String): Savepoint = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
    "setSavepoint not supported"
  )
  ...
  override def createClob(): Clob =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "createClob not supported")

  override def createBlob(): Blob =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "createBlob not supported")
  ...
  override def createSQLXML(): SQLXML = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
  ...
  override def setClientInfo(name: String, value: String): Unit = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
    "setClientInfo not supported"
  )

  override def setClientInfo(properties: Properties): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "properties not supported")

  override def getClientInfo(name: String): String = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
    "getClientInfo not supported"
  )

  override def getClientInfo: Properties = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
    "getClientInfo not supported"
  )
  ...
  override def setSchema(schema: String): Unit = throwWhenClosed {
    if (StringUtils.isBlank(schema)) {
      throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "schema is empty!")
    }
  ...
  override def getSchema: String = throwWhenClosed {
    ...
    if (!resultSet.next()) {
      throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "Get schema failed!")
    }
    resultSet.getString(1)
  }

  override def abort(executor: Executor): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "abort not supported")
  ...
  override def unwrap[T](iface: Class[T]): T =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_CONNECTION, "unwrap not supported")

  override def isWrapperFor(iface: Class[_]): Boolean = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_CONNECTION,
