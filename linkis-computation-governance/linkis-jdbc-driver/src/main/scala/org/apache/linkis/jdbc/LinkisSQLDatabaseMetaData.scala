...
import org.apache.linkis.jdbc.entity.JdbcColumn
import org.apache.linkis.jdbc.utils.JDBCUtils
...
class LinkisSQLDatabaseMetaData(ujesSQLConnection: LinkisSQLConnection)
  ...
  override def getUserName: String =
    if (ujesSQLConnection.getProps.containsKey("user"))
      ...
      throw new LinkisSQLException(LinkisErrorCode.PARAMS_NOT_FOUND, "Missing user information")
  ...
  override def getDriverVersion: String = LinkisSQLDriverMain.DEFAULT_VERSION.toString

  override def getDriverMajorVersion: Int = LinkisSQLDriverMain.DEFAULT_VERSION

  override def getDriverMinorVersion: Int = LinkisSQLDriverMain.DEFAULT_VERSION
  ...
  override def getSQLKeywords: String = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_METADATA,
  ...
  override def getSuperTables(
      ...
      schemaPattern: String,
      tableNamePattern: String
  ): ResultSet = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_METADATA,
  ...
  override def unwrap[T](iface: Class[T]): T =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_METADATA, "unwrap not supported")

  override def isWrapperFor(iface: Class[_]): Boolean =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_METADATA, "isWrapperFor not supported")
