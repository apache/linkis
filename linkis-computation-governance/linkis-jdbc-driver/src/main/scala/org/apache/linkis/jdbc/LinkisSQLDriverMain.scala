...
package org.apache.linkis.jdbc
...
import LinkisSQLDriverMain._

class LinkisSQLDriverMain extends Driver with Logging {
  ...
  override def connect(url: String, properties: Properties): Connection = if (acceptsURL(url)) {
    ...
    val ujesClient = LinkisClientFactory.getUJESClient(props)
    new LinkisSQLConnection(ujesClient, props)
  } else throw new LinkisSQLException(LinkisErrorCode.BAD_URL, "bad url: " + url)
  ...
  private def parseURL(url: String): Properties = {
    ...
    url match {
      case URL_REGEX(host, port, db, params) =>
        ...
        if (StringUtils.isNotBlank(db) && db.length > 1)
          props.setProperty(DB_NAME, db.substring(1))
        if (StringUtils.isNotBlank(params) && params.length > 1) {
          ...
          val kvs = _params.split(PARAM_SPLIT).map(_.split(KV_SPLIT)).filter {
            ...
            case Array(LIMIT, value) =>
              ...
              LinkisSQLDriverMain.LIMIT_ENABLED = value.toLowerCase()
              false
            case Array(key, _) =>
              if (StringUtils.isBlank(key)) {
                throw new LinkisSQLException(
                  LinkisErrorCode.BAD_URL,
                  "bad url for params: " + url
                )
              } else true
            case _ =>
              throw new LinkisSQLException(LinkisErrorCode.BAD_URL, "bad url for params: " + url)
          }
          props.setProperty(PARAMS, kvs.map(_.mkString(KV_SPLIT)).mkString(PARAM_SPLIT))
        }
      case _ => throw new LinkisSQLException(LinkisErrorCode.BAD_URL, "bad url: " + url)
    }
    props
  }
...
object LinkisSQLDriverMain {
  DriverManager.registerDriver(new LinkisSQLDriverMain)
  private val URL_PREFIX = LinkisDriver.URL_PREFIX
  private val URL_REGEX = LinkisDriver.URL_REGEX.r

  val HOST = LinkisDriver.HOST
  val PORT = LinkisDriver.PORT
  val DB_NAME = LinkisDriver.DB_NAME
  val PARAMS = LinkisDriver.PARAMS

  val USER = LinkisDriver.USER
  val TOKEN_KEY = LinkisDriver.TOKEN_KEY
  val TOKEN_VALUE = LinkisDriver.TOKEN_VALUE
  val PASSWORD = LinkisDriver.PASSWORD
  val TABLEAU_SERVER = LinkisDriver.TABLEAU_SERVER
  val LIMIT = LinkisDriver.LIMIT
  var LIMIT_ENABLED = LinkisDriver.LIMIT_ENABLED

  val VERSION = LinkisDriver.VERSION
  val DEFAULT_VERSION = LinkisDriver.DEFAULT_VERSION
  val MAX_CONNECTION_SIZE = LinkisDriver.MAX_CONNECTION_SIZE
  val READ_TIMEOUT = LinkisDriver.READ_TIMEOUT
  val ENABLE_DISCOVERY = LinkisDriver.ENABLE_DISCOVERY
  val ENABLE_LOADBALANCER = LinkisDriver.ENABLE_LOADBALANCER
  val CREATOR = LinkisDriver.CREATOR

  val VARIABLE_HEADER = LinkisDriver.VARIABLE_HEADER
  ...
  def getConnectionParams(
    ...
    if (maxConnectionSize > 0) {
      sb.append(PARAM_SPLIT)
        .append(MAX_CONNECTION_SIZE)
        .append(KV_SPLIT)
        .append(maxConnectionSize)
    }
    if (readTimeout > 0) {
  ...
  private[jdbc] val PARAM_SPLIT = LinkisDriver.PARAM_SPLIT
  private[jdbc] val KV_SPLIT = LinkisDriver.KV_SPLIT
}
