...
class LinkisSQLResultSet(
  ...
  private var resultSetMetaData: LinkisSQLResultSetMetaData = new LinkisSQLResultSetMetaData
  ...
  override def next(): Boolean = {
    ...
    if (null == resultSetRow || currentRowCursor > resultSetRow.size() - 1) {
      if (LinkisSQLDriverMain.LIMIT_ENABLED.equals("false") && !isCompleted) {
        updateResultSet()
        if (isCompleted) {
          return false
        } else {
  ...
  private def evaluate(dataType: String, value: String): Any = {
    if (value == null || value.equals("null") || value.equals("NULL") || value.equals("Null")) {
      ...
      dataType.toLowerCase match {
        case null => throw new LinkisSQLException(LinkisErrorCode.METADATA_EMPTY)
        ...
        case _ =>
          throw new LinkisSQLException(
            LinkisErrorCode.PREPARESTATEMENT_TYPEERROR,
  ...
  private def getColumnValue(columnIndex: Int): Any = {
    if (currentRow == null) {
      throw new LinkisSQLException(LinkisErrorCode.RESULTSET_ROWERROR, "No row found.")
    } else if (currentRow.size() <= 0) {
      throw new LinkisSQLException(
        LinkisErrorCode.RESULTSET_ROWERROR,
  ...
  override def getString(columnIndex: Int): String = {
    ...
    if (wasNull()) {
      throw new LinkisSQLException(LinkisErrorCode.RESULTSET_ROWERROR, "Type is null")
    } else {
      any match {
  ...
  override def getTime(columnIndex: Int): Time = {
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_RESULTSET)
  }
  ...
  override def getMetaData: LinkisSQLResultSetMetaData = {
    if (metaData == null) init()
    resultSetMetaData
  }
  ...
  override def isBeforeFirst: Boolean = {
    if (resultSetRow == null) {
      throw new LinkisSQLException(LinkisErrorCode.RESULTSET_NULL)
  ...
  override def getStatement: Statement = {
    ...
    else throw new LinkisSQLException(LinkisErrorCode.STATEMENT_CLOSED)
  }
  ...
  private def getTimestamp(columnIndex: Int, localTimeZone: DateTimeZone): Timestamp = {
    ...
    if (wasNull()) {
      ...
      new Timestamp(
        TIMESTAMP_FORMATTER.withZone(localTimeZone).parseMillis(String.valueOf(value))
      )
  }
