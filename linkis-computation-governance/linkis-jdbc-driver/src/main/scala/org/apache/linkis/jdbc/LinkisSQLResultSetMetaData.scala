...
class LinkisSQLResultSetMetaData extends ResultSetMetaData with Logging {
  ...
  private[jdbc] def setColumnNameProperties(column: Int, columnName: String): Unit = {
    ...
    } else throw new LinkisSQLException(LinkisErrorCode.METADATA_EMPTY)
  }
  ...
  override def isAutoIncrement(column: Int): Boolean = {
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_METADATA)
  }
  ...
  override def getColumnLabel(column: Int): String = {
    if (columnNameProperties.get(column) == null) {
      throw new LinkisSQLException(LinkisErrorCode.METADATA_EMPTY)
  ...
  override def getPrecision(column: Int): Int = {
    ...
    JdbcColumn.columnPrecision(LinkisSQLTypeParser.parserFromName(name))
  }
  ...
  override def getScale(column: Int): Int = {
    ...
    val name = dataTypeProperties.get(column)
    //    logger.info(s"column is $column and type name is $name")
    JdbcColumn.columnScale(LinkisSQLTypeParser.parserFromName(name))
  }
  ...
  override def getColumnType(column: Int): Int = {
    LinkisSQLTypeParser.parserFromName(getColumnTypeName(column))
  }
