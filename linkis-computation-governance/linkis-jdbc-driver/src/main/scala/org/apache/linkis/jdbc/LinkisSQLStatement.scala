...
import org.apache.linkis.jdbc.hook.JDBCDriverPreExecutionHook
...
class LinkisSQLStatement(private[jdbc] val ujesSQLConnection: LinkisSQLConnection)
  ...
  private var resultSet: LinkisSQLResultSet = _
  ...
  private[jdbc] def throwWhenClosed[T](op: => T): T = ujesSQLConnection.throwWhenClosed {
    if (isClosed) throw new LinkisSQLException(LinkisErrorCode.STATEMENT_CLOSED)
    else op
  }

  override def executeQuery(sql: String): LinkisSQLResultSet = {
    if (!execute(sql)) throw new LinkisSQLException(LinkisErrorCode.RESULTSET_NULL)
    resultSet
  }
  ...
  override def getMaxFieldSize: Int = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_STATEMENT,
  ...
  override def setQueryTimeout(seconds: Int): Unit =
    throwWhenClosed(queryTimeout = seconds * 1000)
  ...
  override def setCursorName(name: String): Unit = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_STATEMENT,
    "setCursorName not supported"
  )

  override def execute(sql: String): Boolean = Utils.tryCatch {
    throwWhenClosed {
      var parsedSQL = sql
      ...
      val action = JobExecuteAction
        .builder()
        .setEngineTypeStr(ujesSQLConnection.getEngineType.toString)
      ...
      jobExecuteResult = Utils.tryCatch(ujesSQLConnection.ujesClient.execute(action.build())) {
        t: Throwable =>
      ...
      if (!status.isCompleted) Utils.tryThrow {
        ...
        case t: TimeoutException =>
          ...
          new LinkisSQLException(LinkisErrorCode.QUERY_TIMEOUT, "query has been timeout!")
            .initCause(t)
        case t => t
      }
      if (!closed) {
        ...
        if (resultSetList != null) {
          resultSet = new LinkisSQLResultSet(resultSetList, this, maxRows, fetchSize)
          true
        } else false
      } else
        throw new LinkisSQLException(LinkisErrorCode.STATEMENT_CLOSED, "Statement is closed.")
    }
  } { case t: Throwable =>
    logger.error("Failed to execute job", t)
    false
  }

  def getJobExcuteResult: JobExecuteResult = jobExecuteResult

  override def getResultSet: LinkisSQLResultSet = resultSet
  ...
  override def addBatch(sql: String): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "addBatch not supported.")

  override def clearBatch(): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "clearBatch not supported.")

  override def executeBatch(): Array[Int] = throw new LinkisSQLException(
    LinkisErrorCode.NOSUPPORT_STATEMENT,
  ...
  override def setPoolable(poolable: Boolean): Unit =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "setPoolable not supported")
  ...
  override def unwrap[T](iface: Class[T]): T =
    throw new LinkisSQLException(LinkisErrorCode.NOSUPPORT_STATEMENT, "unwrap not supported")
