...
public class GenericLabelBuilder extends DefaultGlobalLabelBuilder{
    ...
    public <T extends Label<?>> T build(String labelKey, Object valueObj, Class<?> labelClass,
                                        Type... valueTypes) throws LabelErrorException {
                ...
                if (null != valueObj && List.class.isAssignableFrom(valueObj.getClass())) {
                    ...
                    if (!rawList.isEmpty() && Label.class.isAssignableFrom(rawList.get(0).getClass())) {
                        ...
                        for (Object rawObj : rawList) {
                            genericLabels.add(buildInner(labelKey, ((Label<?>)rawObj).getValue(), suitableLabelClass, suitableValueType));
                        }
                        return (T) genericLabels.stream()
                                .filter((label) -> label != null && label.getValue() != null)
                                .reduce((leftLabel, rightLabel) -> {
