...
package com.apache.wedatasphere.linkis.manager.label.builder.factory;

import com.apache.wedatasphere.linkis.manager.label.builder.*;
...
import com.apache.wedatasphere.linkis.manager.label.exception.LabelRuntimeException;
...
public class StdLabelBuilderFactory implements LabelBuilderFactory {
    ...
    private final LinkedList<ExtensibleLabelBuilder> labelBuilders =
            new LinkedList<>();
    /**
     * Mark sort operation
     */
    ...
    public <T extends Label<?>> T createLabel(String inLabelKey, Object inValueObj, Class<?> outLabelClass, Type... outValueTypes) {
        return processToBuild(inLabelKey, outLabelClass, (builder) -> builder.build(inLabelKey, inValueObj, outLabelClass, outValueTypes));
    }
    ...
    public <T extends Label<?>> T createLabel(String inLabelKey, InputStream inValueStream, Class<?> outLabelClass, Type... outValueTypes) {
        return processToBuild(inLabelKey, outLabelClass, (builder) -> builder.build(inLabelKey, inValueStream, outLabelClass, outValueTypes));
    }
    ...
    public <T extends Label<?>> List<T> getLabels(Map<String, Object> inMap, Class<?> outLabelClass, Type... outValueTypes) {
        return inMap.entrySet().stream()
                .map( entry -> (T)createLabel(entry.getKey(), entry.getValue(), outLabelClass, outValueTypes))
     ...
     * @return
     */
    private <R>R processToBuild(String labelKey,
                                Class<?> labelClass, LabelBuildFunction<R> buildFunc) {
        if(!isSorted){
            sortLabelBuilders();
        }
        ...
        for(ExtensibleLabelBuilder builder : labelBuilders){
            if(builder.canBuild(labelKey, labelClass)){
                try {
                    ...
                    LOG.error("Error Exception in using label builder: [" + builder.getClass().getSimpleName() + "]", error);
                    throw new LabelRuntimeException(error.getErrCode(),
