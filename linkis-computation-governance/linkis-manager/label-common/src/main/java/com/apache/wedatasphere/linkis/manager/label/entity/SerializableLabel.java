...
/**
 * Serialize label value to string value
...
public abstract class SerializableLabel<T> implements Label<T> {
    ...
    public String getStringValue() {
        if(StringUtils.isBlank(stringValue) && null != value) {
            if (LabelUtils.isBasicType(value.getClass())) {
                ...
                //Use jackson default
                Map<String, String> valueMap = LabelUtils.Jackson.convert(value, Map.class, String.class, String.class);
                // Deprecated?
                ...
                List<String> valueSerial = new ArrayList<>(orderedValueNames).stream().map(valueMap::get).filter(Objects::nonNull)
    ...
    public final String toString() {
        return "[key: " + getLabelKey() +
                ", value: " + LabelUtils.Jackson.toJson(value, null)
                + ", str: " + getStringValue() + "]";
    }
