    ...
    protected <V> V transformValueToConcreteType(
        ...
        if (null != rawValue) {
            try {
                ...
                if (concreteClass.isAssignableFrom(sourceClass)) {
                    ...
                    if (concreteClass.equals(Map.class)) {
                        ...
                        try {
                            return (V)
                                    convert(
                                            fromJson(rawString, List.class, Object.class),
                                            javaType);
                        } catch (Exception e) {
                            // Ignore
                ...
                return (V)
                        LabelUtils.Jackson.convert(
                                rawValue, getJavaType(concreteType, parameterTypes));
            } catch (Exception e) {
                // Ignore
        ...
        if (null == valueTypes) {
            valueTypes =
                    findValueTypeFromSuperclass(
                            labelClass, classTypeVariableMap, typeVariableReflect);
        }
        return valueTypes;
    }
            ...
            for (int i = 0; i < interfaces.length; i++) {
                ...
                if (Label.class.isAssignableFrom(interfaceClass)) {
                    ...
                    if (interfaceType instanceof ParameterizedType) {
                        Type[] actualTypes =
                                ((ParameterizedType) interfaceType).getActualTypeArguments();
