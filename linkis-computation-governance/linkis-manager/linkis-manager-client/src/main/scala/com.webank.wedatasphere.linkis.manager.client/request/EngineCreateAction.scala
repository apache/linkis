...
class EngineCreateAction private() extends POSTAction with LinkisManagerAction{
  override def getRequestPayload: String = DWSHttpClient.jacksonJson.writeValueAsString(getRequestPayloads)

  override def suffixURLs: Array[String] = Array("linkisManager","createEngine")
}
object EngineCreateAction {
  def builder(): Builder = new Builder

  class Builder private[EngineCreateAction]() {
    ...
    def build(): EngineCreateAction = {
      val action = new EngineCreateAction()
