...
public enum ManagerCommonErrorCodeSummary {
  ...
  NOT_RESOURCE_POLICY(
      11003,
      "Not supported resource result policy (不支持该资源结果策略)"),
  NOT_RESOURCE_RESULT_TYPE(
      11003,
      "Not supported resource result type(不支持该资源结果类型)"),
  NOT_RESOURCE_TYPE(
      11003,
      "Not supported resource type:{0}(不支持的资源类型)"),
  NOT_RESOURCE_STRING(
      11003,
  ...
  YARN_NOT_EXISTS_QUEUE(
      11006,
      "Queue:{0} is not exists in YARN(YARN 中不存在队列:{0})"),
  ...
  NO_RESOURCE_AVAILABLE(
      11201,
      "No resource available found for em:{0}(没有为 em 找到可用的资源)"),
