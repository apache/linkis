...
object RMConfiguration {
  ...
  val RM_WAIT_EVENT_TIME_OUT = CommonVars("wds.linkis.rm.wait.event.time.out", 1000 * 60 * 12L)

  val RM_REGISTER_INTERVAL_TIME =
    CommonVars("wds.linkis.rm.register.interval.time", 1000 * 60 * 2L)

  val NODE_HEARTBEAT_INTERVAL =
  ...
  val LOCK_RELEASE_TIMEOUT =
    CommonVars("wds.linkis.manager.rm.lock.release.timeout", new TimeType("5m"))

  val LOCK_RELEASE_CHECK_INTERVAL =
    CommonVars("wds.linkis.manager.rm.lock.release.check.interval", new TimeType("5m"))
  ...
  val USER_AVAILABLE_YARN_INSTANCE_MEMORY =
    CommonVars("wds.linkis.rm.yarnqueue.memory.max", new ByteType("450g"))
  ...
  //
  val RM_COMPLETED_SCAN_INTERVAL =
    CommonVars("wds.linkis.rm.module.completed.scan.interval", 10000L)
  ...
  val ALERT_CONTACT_GROUP =
    CommonVars("wds.linkis.rm.alert.contact.group", "q01/hadoop,q02/hadoop")
  ...
  // publics service
  val HIVE_ENGINE_MAINTAIN_TIME_STR =
    CommonVars("wds.linkis.hive.maintain.time.key", "wds.linkis.hive.maintain.time")
