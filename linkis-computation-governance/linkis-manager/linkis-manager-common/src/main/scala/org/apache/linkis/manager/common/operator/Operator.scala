    ...
    parameters.getOrElse(key, defaultVal) match {
      ...
      case v =>
        throw new GovernanceErrorException(
          ...
          MessageFormat.format(UNKNOWN_FOR_KEY.getErrorDesc, v.toString, key)
        )
    }
    ...
    parameters.get(key) match {
      ...
      case Some(t: Any) =>
        throw new GovernanceErrorException(
          ...
          MessageFormat.format(UNKNOWN_CLASS_TYPE_CANNOT_CAST.getErrorDesc, t.toString)
        )
      case None =>
        throw new GovernanceErrorException(
          PARAMETER_OF_IS_NOT_EXISTS.getErrorCode,
          MessageFormat.format(PARAMETER_OF_IS_NOT_EXISTS.getErrorDesc, key)
        )
    }
