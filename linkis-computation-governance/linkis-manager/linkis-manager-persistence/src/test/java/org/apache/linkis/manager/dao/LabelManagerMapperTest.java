...
class LabelManagerMapperTest extends BaseDaoTest {
    ...
    public void registerLabel(){
        ...
        persistenceLabel.setLabelKey("lk");
        persistenceLabel.setStringValue("sv");
        persistenceLabel.setFeature(Feature.CORE);
        persistenceLabel.setLabelValueSize(1);

        labelManagerMapper.registerLabel(persistenceLabel);
        assertTrue(persistenceLabel.getId() >= 1);
    }

    @Test
    public void registerLabelKeyValues(){
        Map<String, String> labelValueKeyAndContent = new HashMap<>();
        labelValueKeyAndContent.put("k1","v1");

        labelManagerMapper.registerLabelKeyValues(labelValueKeyAndContent,1);
    }

    @Test
    public void replaceIntoLabelKeyValue(){
        labelManagerMapper.replaceIntoLabelKeyValue("lk","lsv",1);
    }

    @Test
    public void getLabel(){
        PersistenceLabel label = labelManagerMapper.getLabel(1);
    }
    ...
    public void getLabelByServiceInstance(){
        List<PersistenceLabel> ins = labelManagerMapper.getLabelByServiceInstance("ins");
    ...
    //使用h2以mysql兼容模式会报错。pg使用实体库不会报错
    @Test
    public void deleteLabelResourceByByTicketId(){
//        labelManagerMapper.deleteLabelResourceByByTicketId("ticketId");
    ...
    //使用h2以mysql兼容模式会报错。pg使用实体库不会报错
    @Test
    public void dimListNodeRelationsByKeyValueMap(){
...
//        labelManagerMapper.dimListNodeRelationsByKeyValueMap(labelKeyAndValuesMap,"name");
    }

    //使用h2以mysql兼容模式会报错。pg使用实体库不会报错
    @Test
    public void getNodeRelationsByLabels(){
//        PersistenceLabel persistenceLabel = new PersistenceLabel();
//        persistenceLabel.setLabelKey("lk2");
//        persistenceLabel.setStringValue("sv2");
//        List<PersistenceLabel> labelRels = new LinkedList<>();
//        labelRels.add(persistenceLabel);
//        labelManagerMapper.getNodeRelationsByLabels(labelRels);
    }

    //使用h2以mysql兼容模式会报错。pg使用实体库不会报错
    @Test
    public void listLabelRelationByServiceInstance(){
//        List<ServiceInstance> serviceInstances = new ArrayList<>();
//        ServiceInstance serviceInstance = new ServiceInstance();
//        serviceInstance.setApplicationName("appName");
//        serviceInstance.setInstance("isnt");
...
//        labelManagerMapper.listLabelRelationByServiceInstance(serviceInstances);
    ...
    //使用h2以mysql兼容模式会报错。pg使用实体库不会报错
    @Test
    public void getNodeByLabelKeyValue(){
//        labelManagerMapper.getNodeByLabelKeyValue("sk","sl");
    }
