...
ADD ldh-tars/spark-${SPARK_VERSION}-bin-hadoop2.tgz /opt/ldh/${LINKIS_VERSION}/
...
RUN mkdir -p /etc/ldh \
    ...
    && ln -s /opt/ldh/${LINKIS_VERSION}/spark-${SPARK_VERSION}-bin-hadoop2 /opt/ldh/current/spark \
...
ENV PATH /opt/ldh/current/hadoop/bin:/opt/ldh/current/hive/bin:/opt/ldh/current/spark/bin:/opt/ldh/current/flink/bin:$PATH
