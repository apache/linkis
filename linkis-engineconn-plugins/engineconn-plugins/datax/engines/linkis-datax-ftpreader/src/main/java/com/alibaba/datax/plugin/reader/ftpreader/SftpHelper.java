...
public class SftpHelper extends FtpHelper {
    ...
    public void loginFtpServer(FtpConnParams connParams) {
        ...
        try {
            ...
            session = jsch.getSession(connParams.getUsername(), connParams.getHost(), connParams.getPort());
            ...
            if(StringUtils.isNotBlank(connParams.getPassword())){
                session.setPassword((String) CryptoUtils.string2Object(connParams.getPassword()));
            }
            ...
            // 设置timeout时间
            session.setTimeout(connParams.getTimeout());
            // 通过Session建立链接
            session.connect();
            // 打开SFTP通道
            ...
            if (null != e.getCause()) {
                ...
                String unknownHostException = "java.net.UnknownHostException: " + connParams.getHost();
                String illegalArgumentException = "java.lang.IllegalArgumentException: port out of range:" + connParams.getPort();
                String wrongPort = "java.net.ConnectException: Connection refused";
                if (unknownHostException.equals(cause)) {
                    String message = String.format("请确认ftp服务器地址是否正确，无法连接到地址为: [%s] 的ftp服务器", connParams.getHost());
                    LOG.error(message);
                    ...
                    String message = String.format("请确认连接ftp服务器端口是否正确，错误的端口: [%s] ", connParams.getPort());
                    LOG.error(message);
                    throw DataXException.asDataXException(FtpReaderErrorCode.FAIL_LOGIN, message, e);
                }else{
                ...
                if ("Auth fail".equals(e.getMessage())) {
                    String message = String.format("与ftp服务器建立连接失败,请检查用户名和密码是否正确: [%s]",
                            "message:host =" + connParams.getHost() + ",username = " + connParams.getUsername() + ",port =" + connParams.getPort());
                    LOG.error(message);
    ...
    public long getLastModifyTIme(String filePath) {
        try {
            ...
            String message = String.format("获取SFTP文件: [%s] 最新修改时间异常，请确认是否支持该接口", filePath);
            LOG.error(message);
