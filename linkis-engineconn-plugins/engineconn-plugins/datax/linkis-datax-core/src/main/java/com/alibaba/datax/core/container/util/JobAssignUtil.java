...
public final class JobAssignUtil {
    ...
    public static List<Configuration> assignFairly(Configuration configuration, int channelNumber, int channelsPerTaskGroup) {
        ...
        String writerResourceMark = aTaskConfig.getString(String.format(CoreConstant.JOB_WRITER_PARAMETER, 0) + "." +
                CommonConstant.LOAD_BALANCE_RESOURCE_MARK);
        ...
        // 调整 每个 taskGroup 对应的 RecordChannel 个数（属于优化范畴）
    ...
    private static void adjustChannelNumPerTaskGroup(List<Configuration> taskGroupConfig, int channelNumber) {
        ...
        // 表示有 remainderChannelCount 个 taskGroup,其对应 RecordChannel 个数应该为：avgChannelsPerTaskGroup + 1；
        // （taskGroupNumber - remainderChannelCount）个 taskGroup,其对应 RecordChannel 个数应该为：avgChannelsPerTaskGroup
    ...
    private static LinkedHashMap<String, List<Integer>> parseAndGetResourceMarkAndTaskIdMap(List<Configuration> contentConfig) {
        ...
        for (Configuration aTaskConfig : contentConfig) {
            ...
            List<String> writerResourceMarks = new ArrayList<>();
            List<Object> taskWriterList = aTaskConfig.getList(CoreConstant.JOB_WRITER);
            for(int i = 0; i < taskWriterList.size(); i++){
                writerResourceMarks.add(aTaskConfig.getString(String.format(CoreConstant.JOB_WRITER_PARAMETER, i) +
                        "." + CommonConstant.LOAD_BALANCE_RESOURCE_MARK));
            }
            String[] resourceMarksArray = new String[writerResourceMarks.size()];
            //Join all task writers' resource marks
            String writerResourceMark = StringUtils.join(writerResourceMarks.toArray(resourceMarksArray), "-");
