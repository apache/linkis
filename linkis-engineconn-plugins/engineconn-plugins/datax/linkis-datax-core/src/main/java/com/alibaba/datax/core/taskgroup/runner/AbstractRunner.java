...
public abstract class AbstractRunner {
    ...
    private void mark(State state) {
        if (state == State.SUCCEEDED) {
            this.runnerCommunication.increaseCounter(CommunicationTool.TASK_RUNNING_WRITERS, -1);
            if(this.runnerCommunication.getLongCounter(CommunicationTool.TASK_RUNNING_WRITERS) <= 0){
