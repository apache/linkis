...
public final class DBUtil {
    ...
    private static Map<String, Driver> driverContainer = new HashMap<>();
    ...
    public static String chooseJdbcUrl(final DataBaseType dataBaseType,
                                       ...
                                       final String password, final String proxyHost, final int proxyPort, final List<String> preSql,
                        ...
                        if (StringUtils.isNotBlank(url)) {
                            ...
                            if (null != preSql && !preSql.isEmpty()) {
                                connOK = testConnWithoutRetry(dataBaseType,
                                        url, username, password, preSql, proxyHost, proxyPort);
                            } else {
                                connOK = testConnWithoutRetry(dataBaseType,
                                        url, username, password, checkSlave, proxyHost, proxyPort);
                            }
                            if (connOK) {
                                return url;
                            }
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    public static String chooseJdbcUrlWithoutRetry(final DataBaseType dataBaseType,
                                                   final List<String> jdbcUrls, final String username,
                                                   final String password, final String proxyHost, final int proxyPort,
    ...
    public static boolean checkInsertPrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password,
                                               String proxyHost, int proxyPort,
                                               List<String> tableList) {
        Connection connection = getConnectionWithoutRetry(dataBaseType, jdbcURL, userName, password, proxyHost, proxyPort);
    ...
    public static boolean checkDeletePrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password,
                                               String proxyHost, int proxyPort,
                                               List<String> tableList) {
        Connection connection = getConnectionWithoutRetry(dataBaseType, jdbcURL, userName, password, proxyHost, proxyPort);
    ...
    public static Connection getConnection(final DataBaseType dataBaseType,
        ...
        return getConnection(dataBaseType, jdbcUrl, username, password, String.valueOf(Constant.SOCKET_TIMEOUT_INSECOND * 1000), null);
    }

    public static Connection getConnection(final DataBaseType dataBaseType,
                                           final String jdbcUrl, final String username, final String password, String proxyHost, int proxyPort) {
        Properties props = new Properties();
        if(StringUtils.isNotBlank(proxyHost) && proxyPort > 0) {
            props.put("socksProxyHost", proxyHost);
            props.put("socksProxyPort", String.valueOf(proxyPort));
        }
        return getConnection(dataBaseType, jdbcUrl, username, password, String.valueOf(Constant.SOCKET_TIMEOUT_INSECOND * 1000), props);
    }
     ...
     */
    public static Connection getConnection(final DataBaseType dataBaseType,
                                           final String jdbcUrl, final String username, final String password, final String socketTimeout, Properties defaults) {
            ...
            return RetryUtil.executeWithRetry(new Callable<Connection>() {
                ...
                public Connection call() throws Exception {
                    return DBUtil.connect(dataBaseType, jdbcUrl, username,
                            password, socketTimeout, defaults);
                }
            }, 9, 1000L, true);
        } catch (Exception e) {
            throw DataXException.asDataXException(
    ...
    public static Connection getConnectionWithoutRetry(final DataBaseType dataBaseType,
        ...
        return connect(dataBaseType, jdbcUrl, username, password, String.valueOf(Constant.SOCKET_TIMEOUT_INSECOND * 1000), props);
    }
    public static Connection getConnectionWithoutRetry(final DataBaseType dataBaseType,
                                                       final String jdbcUrl, final String username, final String password, String socketTimeout) {
        return DBUtil.connect(dataBaseType, jdbcUrl, username,
                password, socketTimeout, null);
    }

    private static synchronized Connection connect(DataBaseType dataBaseType,
                                                   String url, String user, String pass) {
        return connect(dataBaseType, url, user, pass, String.valueOf(Constant.SOCKET_TIMEOUT_INSECOND * 1000), null);
    }

    private static synchronized Connection connect(DataBaseType dataBaseType,
                                                   String url, String user, String pass, String socketTimeout, Properties defaultProps) {
        ...
        Properties prop = null != defaultProps? new Properties(defaultProps) : new Properties();
        prop.put("user", user);
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        Driver driver = null;
        try {
            Class<?> clazz = Class.forName(dataBaseType.getDriverClassName(), true, Thread.currentThread().getContextClassLoader());
            driver = new DriverWrapper((Driver)clazz.newInstance());
    ...
    public static List<String> getTableColumns(DataBaseType dataBaseType,
                                               String jdbcUrl, String user, String pass, String proxyHost, int proxyPort, String tableName) {
        Connection conn = getConnection(dataBaseType, jdbcUrl, user, pass, proxyHost, proxyPort);
    ...
    public static boolean testConnWithoutRetry(DataBaseType dataBaseType,
                                               String url, String user, String pass, boolean checkSlave, String proxyHost, int proxyPort) {
        Connection connection = null;

        try {
            connection = getConnectionWithoutRetry(dataBaseType, url, user, pass, proxyHost, proxyPort);
            if (connection != null) {
    ...
    public static boolean testConnWithoutRetry(DataBaseType dataBaseType,
                                               String url, String user, String pass, List<String> preSql, String proxyHost, int proxyPort) {
        Connection connection = null;
        try {
            connection = getConnectionWithoutRetry(dataBaseType, url, user, pass, proxyHost, proxyPort);
            if (null != connection) {
