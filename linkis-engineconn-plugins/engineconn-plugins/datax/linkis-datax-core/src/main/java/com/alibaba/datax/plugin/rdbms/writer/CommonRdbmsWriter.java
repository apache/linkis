...
public class CommonRdbmsWriter {
        ...
        public void privilegeValid(Configuration originalConfig, DataBaseType dataBaseType) {
            ...
            for (int i = 0, len = connections.size(); i < len; i++) {
                ...
                boolean hasInsertPri = DBUtil.checkInsertPrivilege(dataBaseType, jdbcUrl, username, password,
                        proxyHost, proxyPort, expandedTables);
    ...
    public static class Task {
        ...
        public void init(Configuration writerSliceConfig) {
            ...
            this.proxyHost = writerSliceConfig.getString(Key.PROXY_HOST, DEFAULT_PROXY_SOCKS_HOST);
            this.proxyPort = writerSliceConfig.getInt(Key.PROXY_PORT, DEFAULT_PROXY_SOCKS_PORT);
            if(StringUtils.isNotBlank(password)){
                try {
        ...
        public void prepare(Configuration writerSliceConfig) {
            Connection connection = DBUtil.getConnection(this.dataBaseType,
                    this.jdbcUrl, username, password, proxyHost, proxyPort);
