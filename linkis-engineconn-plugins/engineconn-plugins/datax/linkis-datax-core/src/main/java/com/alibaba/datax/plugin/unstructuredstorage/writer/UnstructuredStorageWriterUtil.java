...
public class UnstructuredStorageWriterUtil {
    ...
    public static List<Configuration> split(Configuration writerSliceConfig,
                                            Set<String> originAllFileExists, TransportType transportType, int mandatoryNumber) {
        ...
        for (int i = 0; i < mandatoryNumber; i++) {
            ...
            if(transportType == TransportType.RECORD) {
                ...
                String suffix = CompressSuffixName.chooseSuffix(writerSliceConfig.getString(Key.COMPRESS, ""));
     ...
     * @return
     */
    public static String buildTmpFilePath(String userPath, String tmpPrefix,
                                          char separator, Function<String, Boolean> isExists){
        String tmpFilePath;
        boolean exist;
        do{
            String tmpSuffix = tmpPrefix + UUID.randomUUID().toString().replace('-', '_');
            if(userPath.endsWith(String.valueOf(separator))){
                tmpFilePath = String.format("%s%s%s", userPath,
                        tmpSuffix, separator);
            } else{
                tmpFilePath = String.format("%s%s%s%s", userPath, separator, tmpSuffix, separator);
            }
            Boolean check = isExists.apply(tmpFilePath);
            exist = null != check ? check : false;
        } while(exist);
        return tmpFilePath;
    }
        ...
        try{
            ...
            if(null != compress){
                if("gzip".equalsIgnoreCase(compress)){
                    compressedOutputStream = new GzipCompressorOutputStream(outputStream);
                }else if("bzip2".equalsIgnoreCase(compress)){
                    compressedOutputStream = new BZip2CompressorOutputStream(outputStream);
                }else{
                    throw DataXException.asDataXException(
                            UnstructuredStorageWriterErrorCode.ILLEGAL_VALUE,
                            String.format(
