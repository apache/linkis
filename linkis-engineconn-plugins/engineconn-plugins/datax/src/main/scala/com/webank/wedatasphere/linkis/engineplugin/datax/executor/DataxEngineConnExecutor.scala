...
package com.webank.wedatasphere.linkis.engineplugin.datax.executor
...
import com.webank.wedatasphere.linkis.engineplugin.datax.exception.{DataxCodeErrorException, DataxResouceNotFound}
...
class DataxEngineConnExecutor(id:Int) extends ComputationExecutor{
  ...
  override def executeLine(engineExecutionContext: EngineExecutionContext, code: String): ExecuteResponse = {
    ...
    if (engineExecutionContext != this.engineExecutionContext) {
      ...
      info("Datax Shell executor reset new engineExecutionContext!")
    }
    ...
    try {
      ...
      FileUtils.writeStringToFile(tmpFile,code)
      ...
      if (StringUtils.isNotEmpty(errorLog) || exitCode != 0) {
        ...
        ErrorExecuteResponse("run shell failed", DataxCodeErrorException())
      } else SuccessExecuteResponse()
    } catch {
      case e: Exception => {
        error("Execute datax shell code failed, reason:", e)
        ErrorExecuteResponse("run datax shell failed", e)
      }
      case t: Throwable => ErrorExecuteResponse("Internal error executing datax shell process(执行shell进程内部错误)", t)
    } finally {
  ...
  private def generateRunCode(filename: String): Array[String] = {
    info("exec datax py:"+s"${EngineConnConf.ENGINE_CONN_EXTRA_HOME.getValue}/bin/datax.py"+" filename:"+filename)
    Array("python",s"${EngineConnConf.ENGINE_CONN_EXTRA_HOME.getValue}/bin/datax.py",filename)
  }
