...
package com.webank.wedatasphere.linkis.engineplugin.distcp

import com.webank.wedatasphere.linkis.engineplugin.distcp.builder.DistcpProcessEngineConnLaunchBuilder
import com.webank.wedatasphere.linkis.engineplugin.distcp.factory.DistcpEngineConnFactory
...
class DistcpEngineConnPlugin extends EngineConnPlugin{
  ...
  override def init(params: util.Map[String, Any]): Unit = {
    ...
    typeMap.put("type","distcp")
    typeMap.put("version","3.2.1")
  ...
  override def getEngineConnLaunchBuilder: EngineConnLaunchBuilder = {
    if (null == engineLaunchBuilder) {
      engineLaunchBuilder = new DistcpProcessEngineConnLaunchBuilder
    }
    engineLaunchBuilder
  }
  ...
  override def getEngineConnFactory: EngineConnFactory = EP_CONTEXT_CONSTRUCTOR_LOCK.synchronized {
    if (null == engineFactory) {
      engineFactory = new DistcpEngineConnFactory
    }
    engineFactory
  }
