...
public class ResultUtil {
	...
	public static ChangelogResult createChangelogResult(
			...
			Environment env,
			TableSchema schema,
			ExecutionConfig config) throws SqlExecutionException {
		final TypeInformation[] schemaTypeInfos = Stream.of(schema.getFieldDataTypes())
			.map(TypeInfoDataTypeConverter::fromDataTypeToTypeInfo)
			.toArray(TypeInformation[]::new);
		final RowTypeInfo outputType = new RowTypeInfo(schemaTypeInfos, schema.getFieldNames());
	...
	private static InetAddress getGatewayAddress(DeploymentEntry deploy, Configuration flinkConfig) throws SqlExecutionException {
		...
		if (!address.isEmpty()) {
			try {
				...
				throw new SqlExecutionException("Invalid gateway address '" + address + "' for result retrieval.", e);
			}
		} else {
			// TODO cache this
			...
			if (jobManagerAddress != null && !jobManagerAddress.isEmpty()) {
				try {
					...
					throw new SqlExecutionException("Could not determine address of the gateway for result retrieval " +
