...
public class ChangelogResult<C> extends AbstractResult<C, Tuple2<Boolean, Row>> {
	...
	public ChangelogResult(
			...
			int maxBufferSize) throws SqlExecutionException {
	...
	@Override
	public TypedResult<List<Tuple2<Boolean, Row>>> retrieveChanges() throws JobExecutionException {
	...
	private <T> TypedResult<T> handleMissingResult() throws JobExecutionException {
	...
	private void processRecord(Tuple2<Boolean, Row> change) {
		...
		LOG.debug("Accept the streaming result, row is "+ change.f1.toString());
	}
	...
	private class ResultRetrievalThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				LOG.warn(getName() + " has finished with an error.", e);
			}
			int totalRows = rows;
			if(!changeRecordBuffer.isEmpty()) {
				getFlinkStreamingResultSetListeners().forEach(listener -> listener.onResultSetPulled(changeRecordBuffer.size()));
