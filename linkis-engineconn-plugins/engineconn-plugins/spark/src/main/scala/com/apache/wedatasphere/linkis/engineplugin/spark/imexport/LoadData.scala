...
package com.apache.wedatasphere.linkis.engineplugin.spark.imexport
...
import com.apache.wedatasphere.linkis.engineplugin.spark.imexport.util.{BackGroundServiceUtils, ImExportUtils}
import com.apache.wedatasphere.linkis.hadoop.common.conf.HadoopConf
import com.apache.wedatasphere.linkis.hadoop.common.utils.HDFSUtils
import com.apache.wedatasphere.linkis.storage.excel.XlsUtils
  ...
  def create_table_from_a_file(spark: SparkSession, src: JValue, dest: JValue): Unit = {
    ...
    val df = if (".xlsx".equalsIgnoreCase(suffix)) {
     // info(dateFormatsJson.toString()+ "----------")
      spark.read.format("com.apache.wedatasphere.spark.excel")
