...
public class ExportJobBase extends JobBase {
    ...
    public static boolean isSequenceFiles(Configuration conf, Path p)
            throws IOException {
        return getFileType(conf, p) == ExportJobBase.FileType.SEQUENCE_FILE;
    }
     ...
     * directory)
     */
    public static ExportJobBase.FileType getFileType(Configuration conf, Path p)
            throws IOException {
        ...
        try {
            ...
            if (null == fileStatuses) {
                ...
                return ExportJobBase.FileType.UNKNOWN;
            }
            ...
            if (stat.isDir()) {
                ...
                if (subitems == null || subitems.length == 0) {
                    ...
                    return ExportJobBase.FileType.UNKNOWN; // empty dir.
                }
            ...
            LOG.warn("Input path " + p + " does not exist");
            return ExportJobBase.FileType.UNKNOWN; // doesn't exist!
        }
    }
     ...
     */
    private static ExportJobBase.FileType fromMagicNumber(Path file, Configuration conf) {
        ...
        if (header[0] == 'S' && header[1] == 'E' && header[2] == 'Q') {
            return ExportJobBase.FileType.SEQUENCE_FILE;
        }
        if (header[0] == 'O' && header[1] == 'b' && header[2] == 'j') {
            return ExportJobBase.FileType.AVRO_DATA_FILE;
        }
        return ExportJobBase.FileType.UNKNOWN;
    }
    ...
    protected ExportJobBase.FileType getInputFileType() {
        if (isHCatJob) {
            return ExportJobBase.FileType.HCATALOG_MANAGED_FILE;
        }
        try {
