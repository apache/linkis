...
public enum FlinkErrorCodeSummary implements LinkisErrorCode {
  ...
  CANNOT_FUNCTION_ALREADY(
      16020,
      "Cannot create function:{0}, because a function with this name is already registered(无法创建函数：{0}，因为同名函数已注册)."),
  SQL_CODE_EMPTY(16020, "The sql code is empty(sql 代码为空)"),
  NOT_SUPPORT_RUNTYPE(16020, "Not support the runType:{0}(不支持的runType：{0})."),
  PLANNER_MUST_THESE(16020, "Planner must be one of:{0}(Planner 必须是以下之一:{0})."),
  EXECUTION_MUST_THESE(16020, "Execution type must be one of:{0}(Execution 类型必须是以下之一:{0})."),
  NOT_SUPPORTED_YARNTARGET(16020, "Not supported YarnDeploymentTarget(不支持 YarnDeploymentTarget)"),
  UNKNOWN_CHECKPOINT_MODE(16020, "Unknown checkpoint mode:{0}(未知的 checkpoint 模式:{0})."),
