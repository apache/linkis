...
class HiveEngineConcurrentConnExecutor(id: Int)
  ...
  private val hiveSessionCache: util.Map[String, HiveSession] =
    new ConcurrentHashMap[String, HiveSession]()
  ...
  override def executeLine(
    ...
    val hiveEngineConnFactory = new HiveEngineConnFactory
    val properityMap = new util.HashMap[String, String]()
    for (kv <- engineExecutorContext.getProperties.asScala) {
      LOG.info(s"engineExecutorContext properties key: ${kv._1} value: ${kv._2}")
      properityMap.put(kv._1, kv._2.toString)
    }
    val hiveSession: HiveSession =
      hiveEngineConnFactory.doCreateHiveSession(properityMap)
    hiveSessionCache.put(taskId, hiveSession)

    val hiveConf = hiveSession.hiveConf
          ...
          override def run(): ExecuteResponse = {
            proc match {
              ...
              case _ =>
                ...
                val result = new String(hiveSession.baos.toByteArray)
                logger.info("RESULT => {}", result)
                engineExecutorContext.appendStdout(result)
                hiveSession.baos.reset()
  ...
  def logMemoryCache(): Unit = {
    ...
    for (sessionEntry <- hiveSessionCache.asScala) {
      logger.info(s"running session with taskId : ${sessionEntry._1} .")
    }
  }
  ...
  private def executeHQL(
    ...
    while (needRetry) {
      ...
      try {
        ...
        hiveSessionCache.remove(taskId)
        logMemoryCache()
      }
    }
    if (hasResult) {
  ...
  private def cleanup(taskID: String) = {
    ...
    val hiveSession = hiveSessionCache.get(taskID)
    if (hiveSession == null) LOG.warn(s"do cleanup taskId :${taskID} hiveSession is null")
    else {
      hiveSession.sessionState.deleteTmpOutputFile()
      hiveSession.sessionState.deleteTmpErrOutputFile()
      hiveSession.sessionState.close()
      hiveSessionCache.remove(taskID)
    }
