...
public enum JDBCErrorCodeSummary {
  ...
  JDBC_PARAMS_ILLEGAL(
      26011,
      "JDBC related parameters are illegal(JDBC 相关参数非法)"),
  DRIVER_CLASS_NAME_ERROR(
      26012,
      "The driver class name is required(驱动程序类名是必需的)"),
  UNSUPPORT_JDBC_AUTHENTICATION_TYPES(
      26013,
      "Unsupported jdbc authentication types:{0}(不支持的 jdbc 身份验证类型:{0})"),
  ...
  DOAS_FOR_GET_CONNECTION_ERROR(
      26016,
      "Error in doAs to get one connection(执行 doAs 获取一个连接时出错)"),
  JDBC_USERNAME_NOT_EMPTY(
      26017,
      "The jdbc username cannot be empty(jdbc 用户名不能为空)"),
  JDBC_PASSWORD_NOT_EMPTY(
      26018,
      "The jdbc password cannot be empty(jdbc 密码不能为空)"),
  DATA_SOURCE_INFO_NOT_FOUND(
      26019,
      "Data source info of datasource name:[{0}] not found(数据源：[{0}] 信息未找到)"),
  DATA_SOURCE_NOT_PUBLISHED(
      26020,
      "Data source of datasource name:[{0}] not yet published(数据源：[{0}]尚未发布)"),
  DATA_SOURCE_EXPIRED(
      26021,
      "Data source of datasource name:[{0}] is expired(数据源：[{0}]已过期)"),
  ...
  JDBC_HOST_NOT_NULL(
      26025,
      "The data source jdbc connection host cannot be null(数据源 jdbc 连接主机不能为空)"),
  JDBC_PORT_NOT_NULL(
      26026,
  ...
  UNSUPPORTED_AUTHENTICATION_TYPE(
      26029,
      "Unsupported authentication type:{0}(不支持的身份验证类型)");
