...
public class ShellEngineConnExecutor2 extends ComputationExecutor {
    private static final Logger logger = LoggerFactory.getLogger(ShellEngineConnExecutor2.class);
    ...
    public ShellEngineConnExecutor2(int id) {
        super(id);
        this.id = id;
    }
    ...
    @Override
    public ExecuteResponse executeCompletely(EngineExecutionContext engineExecutionContext, String code,
    ...
    public ExecuteResponse executeLine(EngineExecutionContext engineExecutionContext, String code) {
        ...
        try {
            ...
            String[] argsArr = null;
            if (engineExecutionContext.getTotalParagraph() == 1 && engineExecutionContext.getProperties() != null &&
                    engineExecutionContext.getProperties().containsKey(ShellEngineConnPluginConst.RUNTIME_ARGS_KEY())) {

                ArrayList<String> argsList = (ArrayList<String>) engineExecutionContext.getProperties()
                        .get(ShellEngineConnPluginConst.RUNTIME_ARGS_KEY());

                try {
                    logger.info("Will execute shell task with user-specified arguments: '"
            ...
            if (engineExecutionContext.getTotalParagraph() == 1 && engineExecutionContext.getProperties() != null &&
                ...
                String wdStr = (String) engineExecutionContext.getProperties()
                ...
                try {
                    if (isExecutePathExist(wdStr)) {
                        ...
                        logger.warn("User-specified working-directory: '" + wdStr
            ...
            String[] generatedCode = argsArr == null || argsArr.length == 0 ? generateRunCode(code) : generateRunCodeWithArgs(code, argsArr);
    ...
    private String[] generateRunCodeWithArgs(String code, String[] args) {
        return new String[]{"sh", "-c", "echo \"dummy " + String.join(" ", args) + "\" | xargs sh -c \'" + code + "\'"};
    }
    ...
    public JobProgressInfo[] getProgressInfo(String taskID) {
        ...
        if (progress(taskID) == 0.0f) {
            jobProgressInfo.add(new JobProgressInfo(engineExecutionContext.getJobId().getOrElse(() -> ""), 1, 1, 0, 0));
        } else {
            jobProgressInfo.add(new JobProgressInfo(engineExecutionContext.getJobId().getOrElse(() -> ""), 1, 0, 0, 1));
        }
        return jobProgressInfo.toArray(new JobProgressInfo[0]);
    }
    ...
    public float progress(String taskID) {
        if (this.engineExecutionContext != null) {
            return this.engineExecutionContext.getCurrentParagraph() / (float) this.engineExecutionContext.getTotalParagraph();
        } else {
            return 0.0f;
        }
    }
    ...
    public NodeResource getCurrentNodeResource() {
        ...
        resource.setUsedResource(NodeResourceUtils.applyAsLoadInstanceResource(EngineConnObject.getEngineCreationContext().getOptions()));
        return resource;
    }
