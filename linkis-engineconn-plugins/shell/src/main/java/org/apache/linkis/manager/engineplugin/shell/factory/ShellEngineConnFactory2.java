...
import org.apache.linkis.manager.engineplugin.shell.executor.ShellEngineConnExecutor2;
...
class ShellEngineConnFactory2 implements ComputationSingleExecutorEngineConnFactory {
    private static final Logger logger = LoggerFactory.getLogger(ShellEngineConnFactory2.class);


    @Override
    public LabelExecutor newExecutor(int id, EngineCreationContext engineCreationContext, EngineConn engineConn) {
        if (AccessibleExecutorConfiguration.ENGINECONN_SUPPORT_PARALLELISM()) {
            return new ShellEngineConnConcurrentExecutor(id, ShellEngineConnConf.SHELL_ENGINECONN_CONCURRENT_LIMIT());
        } else {
    ...
    @Override
    public ComputationExecutor newExecutor(int id, EngineCreationContext engineCreationContext, EngineConn engineConn, Label<?>[] labels) {
        return ComputationSingleExecutorEngineConnFactory.super.newExecutor(id, engineCreationContext, engineConn, labels);
    }
    ...
    @Override
    public ComputationExecutor createExecutor(EngineCreationContext engineCreationContext, EngineConn engineConn, Label<?>[] labels) {
        return ComputationSingleExecutorEngineConnFactory.super.createExecutor(engineCreationContext, engineConn, labels);
    }
    ...
    @Override
    public ComputationExecutor createExecutor(EngineCreationContext engineCreationContext, EngineConn engineConn) {
        return ComputationSingleExecutorEngineConnFactory.super.createExecutor(engineCreationContext, engineConn);
    }

    @Override
    public Object createEngineConnSession(EngineCreationContext engineCreationContext) {
        return ComputationSingleExecutorEngineConnFactory.super.createEngineConnSession(engineCreationContext);
    }
