...
class SolrSink extends DataCalcSink[SolrSinkConfig] with Logging {

  def output(spark: SparkSession, ds: Dataset[Row]): Unit = {
    var options = Map(
      "zkhost" -> config.getZkhost,
      "collection" -> config.getCollection,
      "soft_commit_secs" -> config.getSoftCommitSecs,
    ...
    val writer = ds.write.format("solr")
    if (StringUtils.isNotBlank(config.getSaveMode)) {
      writer.mode(config.getSaveMode)
    }
    logger.info(
      s"Save data to solr zkhost: ${config.getZkhost}, collection: ${config.getCollection}"
    )
