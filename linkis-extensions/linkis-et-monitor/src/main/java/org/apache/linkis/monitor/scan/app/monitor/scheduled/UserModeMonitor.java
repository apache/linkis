...
/***
 *  User mode monitoring: regularly trigger scripts to monitor whether the engine status is running normally
 */
@Component
public class UserModeMonitor {
    ...
    @Scheduled(cron = "${linkis.monitor.user.cron}")
    public void job() {
            ...
            userModeStr.forEach(engine -> {
                        ...
                        if (jobInfo.isCompleted()) {
                            if (jobInfo.getJobStatus().equals("Failed")) {
                                logger.info("run fail engineType: {},job id : {}", engine.get("engineType"), jobExecuteResult.taskID());
                            ...
                            logger.info("run timeout engineType: {},job id : {}", engine.get("engineType"), jobExecuteResult.taskID());
    ...
    private static JobExecuteResult toSubmit(LinkedTreeMap<String, String> engine) {
        ...
        labels.put(LabelKeyConstant.ENGINE_TYPE_KEY, engine.get("engineType")); // required engineType Label
        labels.put(LabelKeyConstant.USER_CREATOR_TYPE_KEY, engine.get("executeUser") + "-IDE");// required execute user and creator eg:hadoop-IDE
    ...
    public void dbJob() {
        ...
        GetTableStatisticInfoAction builder = GetTableStatisticInfoAction
                .builder()
                .setUser("hadoop")
                .setDatabase("default")
                .setTable("dual")
