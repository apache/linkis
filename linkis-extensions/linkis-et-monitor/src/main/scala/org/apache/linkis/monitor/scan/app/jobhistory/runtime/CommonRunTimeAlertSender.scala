...
class CommonRunTimeAlertSender()
  extends Observer
    with Logging {
  ...
  override def update(e: Event, jobHistoryList: scala.Any): Unit = {
    ...
    for (a <- jobHistoryList.asInstanceOf[util.List[_]].asScala) {
      if (a == null) {
        ...
        val observeInfoMap = BDPJettyServerHelper.gson.fromJson(jobHistory.getObserveInfo, classOf[java.util.Map[String, String]])
        val extraMap = MapUtils.getMap(observeInfoMap, "extra")
        observeInfoMap.put("title", extraMap.get("title").toString + ",任务id：" + jobHistory.getId + ",执行结果 :" + jobHistory.getStatus)
        observeInfoMap.put("$detail", extraMap.get("detail").toString + ",执行结果 :" + jobHistory.getStatus)
        ...
        val alters = MonitorAlertUtils.getAlertsByDss(Constants.JOB_RESULT_IM, observeInfoMap)
