...
object UserLabelAlertUtils extends Logging {
  ...
  def getAlerts(prefix: String, instans: String): util.Map[String, AlertDesc] = {
    ...
    for ((k: String, v: String) <- properties.asScala) {
      if (ret.containsKey(k)) {
        ...
        val alertInfo =
          new String(new StringBuilder().append(data.alertInfo).toString().getBytes(), "utf-8")
            .replace("$userCreator", instans)
        val receivers = {
