...
class DefaultCodeJobParserTransform extends ParserTransform with Logging {
  ...
  /**
   * This Transform should be the last transform for CodeJob. If no other CodeJobTransform parsed
   * stages for it, then this Transform is used. So, please be noticed, when we build a new
   * [[Parser]], this Transform must be the end one of all CodeJobTransforms.
   * @param in
   *   a AstJob
   * @param context
   *   ASTContext for this Orchestration
   * @return
   */
      ...
      code.split(codeJob.getCodeLogicalUnit.getSeparator).foreach { line =>
        codeLogicalUnits.append(
          new CodeLogicalUnit(
            ...
            codeJob.getCodeLogicalUnit.getLabel,
            codeJob.getCodeLogicalUnit.getSeparator
          )
        )
      }
    }
