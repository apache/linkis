    ...
    Option(execTask).foreach(task => {
      ...
      val event = if (null != progressInfo.get) {
        ...
        progressMap synchronized {
          ...
          TaskRunningInfoEvent(execTask, progressValueSum.asInstanceOf[Float] / progressMap.size.asInstanceOf[Float],
            taskRunningInfoEvent.progressInfo, taskRunningInfoEvent.resourceMap, taskRunningInfoEvent.infoMap)
        }
      } else {
