...
/**
 */
class BaseExecutionTask(maxParallelism: Int, rootExecTask: ExecTask)
    extends ExecutionTask
    with Logging {
  ...
  override def markCompleted(taskCompletedTaskResponse: CompletedTaskResponse): Unit = {
    ...
    logger.info(s"Finished to ExecutionTask(${getId}) with status $getStatus")
  }
