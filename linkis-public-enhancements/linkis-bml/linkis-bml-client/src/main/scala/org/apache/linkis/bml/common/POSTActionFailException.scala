...
case class POSTActionFailException()
    extends ErrorException(70025, "material house client request failed ") {}
...
case class IllegalPathException()
    extends ErrorException(
      70035,
      "The catalog that was passed into the store does not exist or is illegal"
    )
...
case class BmlClientFailException(errorMsg: String)
    extends ErrorException(70081, "An error occurred in the material client")
