...
/**
 * When using the h2 library for testing,some methods that do not support function(on conflict) may report errors,
 * and the pg physical library will not guarantee an error
 * pg使用h2库测试时不支持函数（on conflict）部分方法会报错，pg实体库不会报错
 */
