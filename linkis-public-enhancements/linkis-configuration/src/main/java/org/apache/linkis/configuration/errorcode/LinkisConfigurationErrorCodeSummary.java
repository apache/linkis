...
public enum LinkisConfigurationErrorCodeSummary {
  ...
  CONFIGURATION_NOT_TYPE(
      14100,
      "Configuration does not support engine type:{0}(配置暂不支持{0}引擎类型)"),
  CORRESPONDING_ENGINE_TYPE(
      14100,
      "The corresponding engine type is not matched:{0}(没有匹配到对应的引擎类型:{0})"),
  ...
  CONFIG_KEY_NOT_EXISTS(
      14100, "Config key not exists:{0}(配置键不存在：{0})"),
  LABEL_NOT_EXISTS(14100, "Label not exists:{0}(标签不存在{0})"),
  KEY_OR_VALUE_CANNOT(
      14100, "Key or value cannot be null(键或值不能为空)"),
  PARAMS_CANNOT_BE_EMPTY(
      14100, "Params cannot be empty!(参数不能为空！)"),
  ...
  THE_LABEL_PARAMETER_IS_EMPTY(
      14100, " The label parameter is empty(标签参数为空)"),
  ERROR_VALIDATOR_RANGE(
      14100, "Error validator range！(错误验证器范围！)"),
  TYPE_OF_LABEL_NOT_SUPPORTED(
      14100,
      "This type of label is not supported:{0}(不支持这种类型的标签：{0})");
