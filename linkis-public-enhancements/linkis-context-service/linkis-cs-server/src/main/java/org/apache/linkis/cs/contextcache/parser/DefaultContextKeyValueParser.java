...
public class DefaultContextKeyValueParser implements ContextKeyValueParser {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DefaultContextKeyValueParser.class);
    ...
    public Set<String> parse(ContextKeyValue contextKeyValue) {
        ...
        try {
            if (contextKeyValue != null
                    ...
                    && StringUtils.isNotBlank(contextKeyValue.getContextValue().getKeywords())) {
                String keywordObj = contextKeyValue.getContextValue().getKeywords();

                try {
                    Set<String> keySet =
                            jackson.readValue(keywordObj, new TypeReference<Set<String>>() {});
            ...
            if (null != contextKeyValue
                    && null != contextKeyValue.getContextKey()
                    && StringUtils.isNotBlank(contextKeyValue.getContextKey().getKey())) {
                logger.error(
                        "Failed to parse keywords of " + contextKeyValue.getContextKey().getKey(),
                        e);
            } else {
