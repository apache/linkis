...
public enum LinkisMetadataQueryErrorCodeSummary {
  ...
  FAIL_CLOSE_CONNECTION(
      99900, "Fail to close connection(关闭连接失败),"),
  ERROR_IN_CREATING(
      99900,
      "Error in creating classloader of type:{0}(创建类型的类加载器时出错)"),
  INIT_META_SERVICE(
      99900,
      "Fail to init and load meta service class for type:{0}(无法为以下类型初始化和加载元服务类)"),
  NO_CONSTRUCTOR_SERVICE(
      99900,
      "No public constructor in meta service class:{0}(元服务类中没有公共构造函数)"),
  UNABLE_META_SERVICE(
      99900,
      "Unable to construct meta service class:{0}(无法构建元服务类)"),
  ILLEGAL_META_SERVICE(
      99900,
      "Illegal arguments in constructor of meta service class:{0}(元服务类的构造函数中的非法参数)"),
  FAIL_DOWNLOAD_RESOURCE(
      99900, "Fail to download resource(无法下载资源 )"),
  CANNOT_KEYTAB_PARAMETERS(
      99900,
      "Cannot find the keytab file in connect parameters(在连接参数中找不到密钥表文件)"),
  CANNOT_PARSE_PARAM(
      99900, "Cannot parse the param:{0}(无法解析参数)");
