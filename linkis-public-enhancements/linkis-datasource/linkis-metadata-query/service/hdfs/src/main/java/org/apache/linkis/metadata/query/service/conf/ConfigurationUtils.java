...
public class ConfigurationUtils {
    ...
    private static final CommonVars<String> CONFIG_VIEWFS_LINK_FALLBACK = CommonVars.apply(
            "wds.linkis.server.mdm.hadoop.conf.link.fallback", "linkFallback");

    private static final CommonVars<String> CONFIG_VIEWFS_LINK_NFLY = CommonVars.apply(
            "wds.linkis.server.mdm.hadoop.conf.link.nfly", "linkNfly");
    /**
     * Placeholder
     */
    ...
    public static Map<String, String> filterConfiguration(FileSystem fileSystem, List<String> filterRules, URI uri){
        ...
        List<String> rules = Objects.isNull(filterRules)? new ArrayList<>() : new ArrayList<>(filterRules);
    ...
    private static Function<String, Boolean> addViewFileSystemFilterRules(FileSystem fileSystem, List<String> filerRules, URI uri){
        String mountTableName = Optional.ofNullable(uri)
                .orElse(FileSystem.getDefaultUri(fileSystem.getConf())).getAuthority();
        ...
        if (Objects.nonNull(uri) && fileSystem.getScheme().equals(uri.getScheme())){
            ...
            return config -> (config.startsWith(linkBasicPrefix) &&
