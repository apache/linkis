...
public class HiveMetaService extends AbstractMetaService<HiveConnection> {
    ...
    public MetadataConnection<HiveConnection> getConnection(
        ...
        String principle =
                String.valueOf(
                        params.getOrDefault(HiveParamsMapper.PARAM_HIVE_PRINCIPLE.getValue(), ""));
        HiveConnection conn = null;
        if (StringUtils.isNotBlank(principle)) {
            LOG.info(
                    "Try to connect Hive MetaStore in kerberos with principle:[" + principle + "]");
            ...
            if (StringUtils.isNotBlank(keytabResourceId)) {
                ...
                conn =
                        new HiveConnection(
                                uris, principle, keytabFilePath, getExtraHadoopConf(params));
            } else {
                throw new MetaRuntimeException(
    ...
    private Map<String, String> getExtraHadoopConf(Map<String, Object> connectParams) {
        ...
        Object extraHadoopConfObj =
                connectParams.get(HiveParamsMapper.PARAM_HADOOP_CONF.getValue());
        if (Objects.nonNull(extraHadoopConfObj)) {
            try {
                if (!(extraHadoopConfObj instanceof Map)) {
                    extraHadoopConf =
                            Json.fromJson(
                                    String.valueOf(extraHadoopConfObj),
                                    Map.class,
                                    String.class,
                                    String.class);
                } else {
