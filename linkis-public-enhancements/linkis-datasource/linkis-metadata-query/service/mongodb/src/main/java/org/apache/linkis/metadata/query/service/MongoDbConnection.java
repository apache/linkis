...
public class MongoDbConnection implements Closeable {
    ...
    private static final MongoClientOptions CLIENT_SERVICE_OPTIONS = MongoClientOptions.builder()
            .connectionsPerHost(5)
            .socketTimeout(6000)
            .connectTimeout(3000).serverSelectionTimeout(50).build();
    ...
    public List<MetaColumnInfo> getColumns(String database, String collection)
        ...
        MongoCursor<Document> dbCursor = collection1.find().sort(descending("_id")).batchSize(DOC_FETCH_LIMIT)
                .limit(DOC_FETCH_LIMIT).iterator();
    ...
    private MongoClient getDBConnection(ConnectMessage connectMessage, String database) throws Exception {
        ...
        LOG.info("mongo information is database:{}, username:{}, passwordd:{} ", database, connectMessage.username, connectMessage.password);

        MongoClient client = null;
        try {
            MongoCredential credential = MongoCredential.createCredential(connectMessage.username, database, connectMessage.password.toCharArray());
            ...
            throw new RuntimeException("Fail to authenticate to Mongo server: [ host: " + connectMessage.host + ", port: " +
                    connectMessage.port + ", username: " + connectMessage.username + "]", e);
