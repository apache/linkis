  ...
  def dealDDLExecuteResponse(ddlExecuteResponse: DDLExecuteResponse): DDLCompleteResponse = {
    ...
    if (!ddlExecuteResponse.status) {
      ...
      Utils.tryCatch {
        ...
        case e: Exception => logger.error(s"fail to persist table for user ${ddlExecuteResponse.user}", e)
          DDLCompleteResponse(false)
        case t: Throwable => logger.error(s"fail to persist table for user ${ddlExecuteResponse.user}", t)
          DDLCompleteResponse(false)
      }
    }
  }
