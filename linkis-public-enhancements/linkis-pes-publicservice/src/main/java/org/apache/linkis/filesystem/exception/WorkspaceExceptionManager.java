...
public class WorkspaceExceptionManager {
  ...
  private static final Map<String, String> desc = initialDescMap();

  private static HashMap<String, String> initialDescMap() {
    ...
    map.put("80004", "The path:{0} is empty(路径:{0} 为空)!");
    map.put("80005", "The created folder name:{0} is duplicated(创建的文件夹名:{0} 重复)");
    map.put("80006", "The file name:{0} created is duplicated(创建的文件名:{0} 重复)");
    map.put("80007", "The renamed name:{0} is repeated,(重命名的名字:{0} 重复)");
    map.put("80008", "The deleted file or folder does not exist(删除的文件or文件夹不存在)!");
    map.put(
        "80009",
    ...
    map.put("80011", "Downloaded file: {0} does not exist(下载的文件:{0}不存在)");
    map.put("80012", "This user has no permission to read this file(该用户无权读取该文件)!");
    map.put("80013", "File: {0} does not exist(文件:{0}不存在)");
    map.put(
        "80014",
        "The user has no permission to modify the contents of this file and cannot save it(该用户无权限对此文件内容进行修改，无法保存)!");
    map.put("80015", "Unsupported resultset output type(不支持的结果集输出类型)");
    map.put("80016", "The file content is empty and cannot be imported(文件内容为空，不能进行导入操作)!");
    map.put(
        "80017",
    ...
    map.put("80018", "This user has no permission to read this log(该用户无权限读取此日志)!");
    map.put("80019", "ScriptContent is empty,this is normal(scriptContent 为空，这是正常的)!");
    map.put("80021", "Upload failed(上传失败)");
    map.put("80022", "Update failed(更新失败)");
    map.put("80023", "Download failed(下载失败)");
    map.put(
        "80024", "Non-tabular result sets cannot be downloaded as excel(非table类型的结果集不能下载为excel)");
    map.put(
        "80028",
        "The path exist special char,only support numbers, uppercase letters, underscores, Chinese(路径存在特殊字符,只支持数字,字母大小写,下划线,中文)");
    map.put("80029", "Empty dir(空目录)!");
    map.put("80030", "Creating user path: {0} failed(创建用户路径:{0}失败)");
    map.put("80031", "User: {0} not initialized(用户:{0}未初始化)");
    map.put(
        "80032",
    ...
    map.put("80039", "File upload failed, error message: {0} (文件上传失败，错误信息：{0})");
    map.put("80040", "{0} file not found in the archive ({0}文件不存在，请确认包中包含{0}文件)");
    return map;
  }
