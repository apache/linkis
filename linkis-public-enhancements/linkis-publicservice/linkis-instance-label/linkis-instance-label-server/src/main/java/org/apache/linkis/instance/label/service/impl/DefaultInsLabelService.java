...
public class DefaultInsLabelService implements InsLabelAccessService {
    ...
    public void attachLabelsToInstance(
        ...
        batchOperation(
                insLabelIds,
                subInsLabelIds ->
                        insLabelRelationDao.insertRelations(
                                serviceInstance.getInstance(), subInsLabelIds),
    ...
    public List<ServiceInstance> searchLabelRelatedInstances(ServiceInstance serviceInstance) {
        if (null != serviceInstance) {
            return insLabelRelationDao
                    .searchLabelRelatedInstances(new InstanceInfo(serviceInstance)).stream()
                    .map(instanceInfo -> (ServiceInstance) instanceInfo)
                    .collect(Collectors.toList());
        }
        return Collections.emptyList();
        ...
        insLabels.forEach(
                insLabel -> {
                    if (Optional.ofNullable(insLabel.getId()).isPresent()) {
                        ...
                        insLabel =
                                instanceLabelDao.searchForUpdate(
                                        insLabel.getLabelKey(), insLabel.getStringValue());
                    }
                    if (null != insLabel) {
                        ...
                        if (null == exist) {
                            LOG.info(
                                    "Remove information of instance label: ["
                                            + insLabel.toString()
                                            + "]");
    ...
    private List<InsPersistenceLabel> filterLabelNeededInsert(
        ...
        List<InsPersistenceLabelSearchVo> labelSearchVos =
                insLabels.stream()
                        .map(InsPersistenceLabelSearchVo::new)
                        .collect(Collectors.toList());
