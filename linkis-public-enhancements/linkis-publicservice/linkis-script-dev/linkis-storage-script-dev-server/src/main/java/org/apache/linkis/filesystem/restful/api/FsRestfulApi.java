...
public class FsRestfulApi {
    ...
    public Message deleteDirOrFile(HttpServletRequest req, @RequestBody JsonNode json)
        ...
        if (!fileSystem.canWrite(fsPath.getParent())
                || !fileSystem.canExecute(fsPath.getParent())) {
        ...
        if (fsPathListWithError != null) {
            for (FsPath children : fsPathListWithError.getFsPaths()) {
                ...
                if (!children.isdir()) {
                    ...
                    dirFileTreeChildren
                            .getProperties()
                            .put("size", String.valueOf(children.getLength()));
                    dirFileTreeChildren
                            .getProperties()
    ...
    public void download(
        ...
        try {
            ...
            response.addHeader(
                    "Content-Disposition", "attachment;filename=" + new File(path).getName());
    ...
    public Message saveScript(HttpServletRequest req, @RequestBody Map<String, Object> json)
        ...
        try (ScriptFsWriter scriptFsWriter =
                ScriptFsWriter.getScriptFsWriter(
                        fsPath, charset, fileSystem.write(fsPath, true)); ) {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "outputFileType", required = false,dataType = "String", value = "输出文件类型"),
            @ApiImplicitParam(name = "csvSeperator", required = false,dataType = "String", value = "csv分隔栏"),
            @ApiImplicitParam(name = "quoteRetouchEnable", required = false, dataType = "boolean", value = "是否引用修饰"),
            @ApiImplicitParam(name = "outputFileName", required = false,dataType = "String", value = "输出文件名称"),
            @ApiImplicitParam(name = "sheetName",required = false, dataType = "String", value = "sheet名称"),
            ...
            @ApiImplicitParam(name = "autoFormat",required = false, dataType = "Boolean", value = "是否自动")
    })
    ...
    public void resultsetToExcel(
            ...
            @RequestParam(value = "outputFileType", required = false,defaultValue = "csv") String outputFileType,
            @RequestParam(value = "csvSeperator", required = false,defaultValue = ",") String csvSeperator,
        ...
        try {
            ...
            fileSource =
                    FileSource$.MODULE$
                            .create(fsPath, fileSystem)
                            .addParams("nullValue", nullValue);
            switch (outputFileType) {
                case "csv":
                    if (FileSource$.MODULE$.isTableResultSet(fileSource)) {
                        ...
                        fsWriter =
                                ScriptFsWriter.getScriptFsWriter(
                                        new FsPath(outputFileType), charset, outputStream);
                    }
    ...
    public void resultsetsToExcel(
        ...
        try {
            ...
            response.addHeader(
                    ...
                    "attachment;filename="
                            + new String(
                                    outputFileName.getBytes(StandardCharsets.UTF_8), "ISO8859-1")
                            + ".xlsx");
    ...
    public Message openLog(
        ...
        try (FileSource fileSource =
            ...
            snd.stream()
                    ...
                    .forEach(
                            s ->
                                    WorkspaceUtil.logMatch(s, start)
                                            .forEach(i -> log[i].append(s).append("\n")));
            return Message.ok()
                    .data(
                            "log",
                            Arrays.stream(log).map(StringBuilder::toString).toArray(String[]::new));
        }
    }
