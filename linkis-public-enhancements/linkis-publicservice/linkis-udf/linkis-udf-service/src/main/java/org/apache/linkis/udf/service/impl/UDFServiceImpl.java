...
public class UDFServiceImpl implements UDFService {
    ...
    private void validateJarFile(UDFInfo udfInfo, String userName) throws UDFException {
        ...
        try {
            ...
            logger.error(e);
            throw new UDFException(
                    "Verify that there is a problem with the UDF jar package(校验UDF jar包存在问题)："
                            + e.getMessage());
        }
        ...
        try {
            URL[] url = {
                new URL("file://" + targetFile.getAbsolutePath()),
            ...
            throw new UDFException(
                    "There is a problem verifying the UDF jar package: the class is not found(校验UDF jar包存在问题：找不到类) "
                            + cne.getMessage());
        } catch (NoSuchMethodException e) {
            throw new UDFException(
                    "There is a problem verifying the UDF jar package: class(校验UDF jar包存在问题：类) "
                            + udfClassName
                            + " Missing public no-argument constructor(缺少public的无参数构造方法)");
        } catch (Exception e) {
            throw new UDFException(
                    "Verify that there is a problem with the UDF jar package(校验UDF jar包存在问题)："
                            + e.getMessage());
    ...
    public UDFInfo updateUDF(UDFInfo udfInfo, String userName) throws UDFException {
        ...
        if (userName.equals(oldUdfInfo.getCreateUser())) {
            ...
            Long sharedUdfId =
                    udfDao.selectAllShareUDFInfoIdByUDFId(userName, oldUdfInfo.getUdfName());
            if (sharedUdfId == null) {
            ...
            if (sharedUdfId != null) {
                ...
                try {
                    UDFInfo newSharedUdf =
                            createSharedUdfInfo(udfInfo, originalSharedUdf.getTreeId(), sharedPath);
    ...
    public FsPath copySharedUdfFile(String userName, UDFInfo udfInfo) throws IOException {
        String fileName =
                FilenameUtils.getBaseName(udfInfo.getPath())
                        + System.currentTimeMillis()
                        + "."
                        + FilenameUtils.getExtension(udfInfo.getPath());
        FsPath udfPath = FsPath.getFsPath(udfInfo.getPath());
        FsPath sharedPath =
                FsPath.getFsPath(UdfConfiguration.UDF_SHARE_PATH().getValue(), fileName);
    ...
    public Boolean deleteUDF(Long id, String userName) throws UDFException {
        ...
        if (udfInfo != null) {
            Long sharedUdfId =
                    udfDao.selectAllShareUDFInfoIdByUDFId(userName, udfInfo.getUdfName());
            if (sharedUdfId != null) {
                if (sharedUdfId == udfInfo.getId()) {
                ...
                if (!StringUtils.isEmpty(jarPath)) {
                    ...
                    for (UDFInfo udf : sameJarUDF) {
                        ...
                        if (jarPath.equalsIgnoreCase(
                                udf.getPath().substring(udf.getPath().lastIndexOf("/")))) {
                            throw new UDFException(
                                    "There is a Jar package with a different path of the same name. The UDF name is(存在同名不同路径的Jar包,UDF名字为)："
                                            + udf.getUdfName()
                                            + ",The jar path is(jar路径为)"
                                            + udf.getPath());
                        }
                    }
                }
