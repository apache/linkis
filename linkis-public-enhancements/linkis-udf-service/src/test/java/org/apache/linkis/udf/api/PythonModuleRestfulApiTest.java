...
public class PythonModuleRestfulApiTest {
    ...
    public void testPythonList() throws Exception {
        // 测试获取Python模块列表
        mockMvc.perform(get("/python-list")
    ...
    public void testPythonDelete() throws Exception {
        // 测试删除Python模块
        mockMvc.perform(get("/python-delete")
                        .param("id", "1")
                        .param("isExpire", "0"))
                .andExpect(status().isOk());
    ...
    public void testPythonSave() throws Exception {
        // 测试保存Python模块
        mockMvc.perform(post("/python-save")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"testModule\",\"path\":\"/path/to/module.py\",\"engineType\":\"python\",\"isLoad\":1,\"isExpire\":0}"))
                .andExpect(status().isOk());

        // 测试保存Python模块时传入空名称
        mockMvc.perform(post("/python-save")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"\",\"path\":\"/path/to/module.py\",\"engineType\":\"python\",\"isLoad\":1,\"isExpire\":0}"))
                .andExpect(status().isBadRequest());

        // 测试保存Python模块时传入空路径
        mockMvc.perform(post("/python-save")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"testModule\",\"path\":\"\",\"engineType\":\"python\",\"isLoad\":1,\"isExpire\":0}"))
                .andExpect(status().isBadRequest());
    }
    ...
    public void testPythonFileExist() throws Exception {
        // 测试检查Python模块文件是否存在
        mockMvc.perform(get("/python-file-exist")
                        .param("fileName", "testModule.py"))
                .andExpect(status().isOk());
