...
public class CachedTokenServiceTest {
    ...
    public void isTokenAcceptableWithHost() throws Exception {
        if (service != null) {
            Assert.assertTrue(service.isTokenAcceptableWithHost("test1", "10.107.108.126"));
            Assert.assertFalse(service.isTokenAcceptableWithHost("test1", "10.107.108.1266"));
            Assert.assertTrue(service.isTokenAcceptableWithHost("test3", "10.107.108.126"));
        }
    }
    ...
    public void doAuth() throws Exception {
        if (service != null) {
            Assert.assertTrue(service.doAuth("test1", "hduser05", "10.107.108.126"));
            Assert.assertTrue(service.doAuth("test2", "hduser05", "10.107.108.126"));
            Assert.assertTrue(service.doAuth("test3", "hduser05", "10.107.108.126"));
            try {
                service.doAuth("test1", "hduser05", "10.107.108.1266");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15203);
            }
            try {
                service.doAuth("test2", "hduser05", "10.107.108.1266");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15203);
            }
            Assert.assertTrue(service.doAuth("test3", "hduser05", "10.107.108.1266"));
            try {
                service.doAuth("test1", "hduser055", "10.107.108.126");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15202);
            }
            Assert.assertTrue(service.doAuth("test2", "hduser055", "10.107.108.126"));
            try {
                service.doAuth("test3", "hduser055", "10.107.108.126");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15202);
            }
            try {
                service.doAuth("test4", "hduser05", "10.107.108.126");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15201);
            }
            try {
                service.doAuth("xxxx", "hduser05", "10.107.108.126");
            } catch (Exception e) {
                Assert.assertEquals(((TokenAuthException) e).getErrCode(), 15201);
            }
        }
