...
public class GatewayAuthorizationFilter implements GlobalFilter, Ordered {
    ...
    private String getRequestBody(ServerWebExchange exchange) {
        ...
        try {
            ...
            GatewayWarnException exception =
                    new GatewayWarnException(18000, "get requestBody failed!");
    ...
    private Route getRealRoute(Route route, ServiceInstance serviceInstance) {
        ...
        if (routeUri.startsWith(
                SpringCloudGatewayConfiguration.ROUTE_URI_FOR_WEB_SOCKET_HEADER())) {
            scheme = SpringCloudGatewayConfiguration.ROUTE_URI_FOR_WEB_SOCKET_HEADER();
        } else if (routeUri.startsWith(
                SpringCloudGatewayConfiguration.ROUTE_URI_FOR_HTTP_HEADER())) {
    ...
    private Mono<Void> gatewayDeal(
        ...
        ServerHttpRequest.Builder builder =
                exchange.getRequest()
                        .mutate()
                        .headers(SpringCloudHttpUtils::addIgnoreTimeoutSignal);
        if (!((SpringCloudGatewayHttpRequest) gatewayContext.getRequest())
                .getAddCookies()
                .isEmpty()) {
            builder.headers(
                    httpHeaders -> {
