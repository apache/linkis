...
class Globalcache extends Basic {
     ...
     */
    async setCache(args) {
        const cache = await this.getCache(args.key);
     ...
     */
    async getCache(key) {
        const caches = (await this.get(key)) || [];
        return caches[0];
    }
    ...
    async removeCache(args) {
        const cache = await this.getCache(args.id);
        let tabList = [];
        if (!isNil(cache)) {
            tabList = cache.tabList;
            remove(tabList, (n) => n === args.tabId);
        }
        return this.update(args.id, { key: args.id, tabList });
    }
