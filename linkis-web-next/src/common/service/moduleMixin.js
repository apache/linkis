...
export default function mixinDispatch (requireComponent, requireComponentVue) {
    ...
    if (requireComponentVue) {
        requireComponentVue.keys().forEach((fileName) => {
            ...
            const indexPath = `./${  fileName.split('/')[1]  }/index.js`;
            const moduleConfig = modules[indexPath];
            const mixin = moduleConfig && moduleConfig.mixin;
            if (!vueConfig.mixins) {
                vueConfig.mixins = [mixin];
            } else if (vueConfig.mixins.indexOf(mixin) == -1) {
