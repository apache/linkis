  ...
  <template>
    <div class="sub-tabs">
        ...
        <FTabs @change="onTaskDetailTabChange" class="task-log-tabs" type="card" v-model="activeTaskLogTabPane">
            ...
            <FTabPane v-if="['hive', 'spark'].includes(task.engineType)" name="yarnApp" value="yarnApp" />
        </FTabs>
        <WeEditor
...
const handleDownloadLogs = () => {
    FModal.confirm({
        ...
        onOk() {
            const url = `/api/rest_j/v1/engineconnManager/downloadEngineLog?emInstance=${props.params.ecmInstance}&instance=${props.task.engineInstance}&logDirSuffix=${encodeURIComponent(logPath.value)}&logType=${activeTaskLogTabPane.value}`;
...
const handleInitiation = async (fromLine: number) => {
    if(props.task && props.params.ecmInstance) {
        try {
          let res = await api.fetch('/linkisManager/openEngineLog', {
            applicationName: 'linkis-cg-engineconn',
            emInstance: props.params.ecmInstance,
            instance: props.task.engineInstance,
            parameters: {
                pageSize: 1000,
                fromLine,
                logType: activeTaskLogTabPane.value,
                logDirSuffix: props.params.logDirSuffix,
            }
          }, 'post') || {};
          if (res && res.result) {
