...
export class Main extends LetgoPageBase {
    ...
    showDeleteConfirmation (selectedModule) {
        ...
        this.$utils.FModal.confirm({
            title: '确认删除',
            content: `您确定要删除模块"${selectedModule.name}"吗？`,
            mask: false,
            onOk: async () => {
                try {
    ...
    showLoadStatusChangeConfirmation (selectedModule) {
        ...
        this.$utils.FModal.confirm({
            title: '确认加载状态变更',
            mask: false,
            content: `您确定要修改模块 ${selectedModule.name} 的加载状态吗？`,
            onOk: async () => {
    ...
    async validateModuleName (newModuleName) {
        if (newModuleName.split('.')[0].length > 50) {
            this.$utils.FMessage.error('模块名称长度不能超过50个字符');
            throw new Error('模块名称长度不能超过50个字符');
        }
        // 名称只支持数字字母下划线，且以字母开头
        if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(newModuleName.split('.')[0])) {
            this.$utils.FMessage.error('模块名称只支持数字字母下划线，且以字母开头');
            throw new Error('模块名称只支持数字字母下划线，且以字母开头');
        }
        try {
            ...
            // this.$utils.FMessage.error(`模块名称${newModuleName}已存在，如需重新上传请先删除旧的模块`);
            throw new Error(`模块名称${newModuleName}已存在，如需重新上传请先删除旧的模块`);
        }
    }

    async validateModuleSize (size) {
        if (size > 52428800) {
            this.$utils.FMessage.error('模块大小不能超过50MB');
            throw new Error('模块大小不能超过50MB');
        }
    }
    ...
    async handleDeleteModule () {
        try {
            if (this.selectedModuleId === null) {
                throw new Error('模块ID未设置，无法执行删除操作');
            }
