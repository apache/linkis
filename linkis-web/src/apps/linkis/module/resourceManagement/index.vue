  ...
  <div class="resource-page">
    ...
    <div class="resource-title" :class="{ 'admin-title': !isAdminModel }">
      ...
      <Form
        ...
        :style="{ float: isAdminModel ? 'none' : 'right' }"
        :model="searchBar"
        inline
      >
        <FormItem
        ...
        <FormItem
          v-show="isAdminModel"
          prop="status"
          :label="$t('message.linkis.formItems.engineType')"
        >
        ...
        <FormItem>
          ...
          >
          <Button type="primary" @click="switchAdmin">{{
            isAdminModel
          ...
          >{{
            showOperations
              ? $t('message.linkis.hide')
              : $t('message.linkis.showOperations')
          }}</Button
          >
          <Button
            ...
            "
            style="margin-left: 10px"
          >{{ $t('message.linkis.resetAll') }}</Button
          >
        </FormItem>
      </Form>
    </div>
    <div class="noData" v-if="!isAdminModel && progressDataList.length <= 0">
      No data, try refresh
    </div>
    <!-- progress bar(进度条) -->
    ...
    <template v-if="isShowTable && !isAdminModel">
      ...
      <div class="appListTag">
        <div class="tagName">
          <span class="span-white-space"
          >{{ $t('message.linkis.tableColumns.label') }}:</span
          >
          <Tag v-for="(item, index) in tagTitle" :key="index" color="primary">{{
            item
          }}</Tag>
        </div>
        <div class="resourceList">
          <span class="span-white-space"
          >{{ $t('message.linkis.resources') }}:</span
          >
          <span v-if="applicationList.usedResource">
            <Tag color="success"
            >{{ `${calcCompany(applicationList.usedResource)}` }}(used)</Tag
            >
            <Tag color="error"
            >{{ `${calcCompany(applicationList.maxResource)}` }}(max)</Tag
            >
            <Tag color="warning" v-if="applicationList.leftResource"
            >{{ `${calcCompany(applicationList.leftResource)}` }}(remain)</Tag
            >
          </span>
        </div>
        <div class="instanceNum">
          <span class="span-white-space"
          >{{ $t('message.linkis.instanceNum') }}:</span
          >
          <span v-if="applicationList.usedResource"
          >{{ applicationList.usedResource.instance }} /
            {{ applicationList.maxResource.instance }}</span
          >
        </div>
      </div>
      <Table
        ...
        <template slot-scope="{ row }" slot="usedResource">
          <span>{{
            `${calcCompanyAdmin(
              ...
              'usedResource',
              'memory'
            )},${calcCompanyAdmin(row, 'usedResource', 'instance')}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="lockedResource">
          <span>{{
            `${calcCompanyAdmin(
              ...
              'lockedResource',
              'memory'
            )},${calcCompanyAdmin(row, 'lockedResource', 'instance')}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="maxResource">
          <span>{{
            `${calcCompanyAdmin(
              ...
              'maxResource',
              'memory'
            )},${calcCompanyAdmin(row, 'maxResource', 'instance')}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="leftResource">
          <span :class="'label-' + row.loadResourceStatus">{{
            `${calcCompanyAdmin(
              row,
              'leftResource',
              'cores'
              ...
              'leftResource',
              'memory'
            )},${calcCompanyAdmin(row, 'leftResource', 'instance')}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="yarnUsedResource">
          <span>{{
            `${calcCompanyAdmin(
              ...
              'memory',
              true
            )},${calcCompanyAdmin(row, 'usedResource', 'instance', true)}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="yarnMaxResource">
          <span>{{
            `${calcCompanyAdmin(
              ...
              'memory',
              true
            )},${calcCompanyAdmin(row, 'maxResource', 'instance', true)}`
          }}</span>
        </template>
        <template slot-scope="{ row }" slot="yarnLeftResource">
          <span :class="'label-' + row.queueResourceStatus">{{
            `${calcCompanyAdmin(
              row,
              'leftResource',
              'cores',
              ...
              'memory',
              true
            )},${calcCompanyAdmin(row, 'leftResource', 'instance', true)}`
          }}</span>
        </template>
      </Table>
      <div class="page-bar">
...
export default {
                    ...
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          ...
                          api
                            .fetch(`/linkisManager/rm/enginekill`, data)
                            .then(() => {
  ...
  methods: {
    ...
    async initExpandList() {
      ...
      try {
        let engines =
          (await api.fetch('/linkisManager/rm/userresources', 'post')) || {}
    ...
    async expandChange(item, isChildren = false, parentData = {}) {
      ...
      if (!isChildren) {
        ...
        this.tagTitle = Object.assign(this.tagTitle, [
          parentData.userCreator,
          item.engineType
        ])
      }
      let parameter = {
        userCreator: isChildren
          ? parentData.userCreator
          : this.currentParentsData.userCreator,
        engineType: isChildren ? this.currentEngineData.engineType : undefined
      }
      try {
        let engines =
          (await api.fetch(
            '/linkisManager/rm/applicationlist',
            parameter,
            'post'
          )) || {}
        this.loading = false
        ...
        if (this.stopList.length && tableData.length)
    ...
    timeFormat(row) {
      if (row.startTime.indexOf('CST') > 0) {
        return moment(new Date(row.startTime))
          .utc()
          .subtract(6, 'hour')
    ...
    calcCompany(resource) {
      ...
      return resource &&
        (resource.cores !== undefined ||
          resource.memory !== undefined ||
          resource.instance !== undefined)
        ? `${calcCompanyToData(resource.cores)}cores,${calcCompanyToData(
          resource.memory,
          true
        )}G,${calcCompanyToData(resource.instance)}apps`
        : ''
    },
    calcCompanyAdmin(row, field, type, yarn) {
      ...
      if (row.resourceType === 'LoadInstance') {
        ...
        if (yarn) {
          ...
          // }
          data =
            row[field].DriverAndYarnResource.loadInstanceResource[type] ||
    ...
    search() {
      const param = {
        ...
        engineType:
          this.searchBar.engineType === 'all' ? '' : this.searchBar.engineType,
        page: this.page.pageNow,
        size: this.page.pageSize
      }
      api
        .fetch('/linkisManager/rm/allUserResource', param, 'get')
        .then((res) => {
      ...
      this.$Modal.confirm({
        ...
        content:
          resourceId !== undefined
            ? this.$t('message.linkis.resetTip')
            : this.$t('message.linkis.resetAllTip'),
        onOk: () => {
          const param = resourceId !== undefined ? { resourceId } : {}
          api
            .fetch('linkisManager/rm/resetResource', param, 'delete')
            .then(() => {
