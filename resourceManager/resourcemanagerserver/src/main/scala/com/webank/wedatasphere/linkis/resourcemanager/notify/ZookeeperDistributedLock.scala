...
class ZookeeperDistributedLock(zk: ZooKeeper, var lockBase: String, lockName: String) extends DistributedLock with Logging {
  ...
  try
      if (zk.exists(lockBase, true) == null) zk.create(lockBase, "".getBytes, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT)
  catch {
  ...
  override def acquire(time: Long, unit: TimeUnit): Boolean = try {
  ...
  private def doAcquire(time: Long, unit: TimeUnit, waitingNodeName: String): Boolean = {
    ...
    if (stat == null) return doAcquire(time, unit, waitingNodeName) // Nodes has been updated, try again else if (!waitingBell.await(time, unit)) {
    try {
...
private class DirectFrontWatcher(waitingNodeName: String) extends Watcher {
  ...
  override def process(watchedEvent: WatchedEvent): Unit = {
    if (watchedEvent.getType.equals(EventType.NodeDeleted)) watchDirectFrontNode(waitingNodeName)
  }
  ...
  private def watchDirectFrontNode(waitingNodeName: String): Unit = {
    ...
    val stat = zk.exists(lockBase + "/" + directFrontNode, new DirectFrontWatcher(waitingNodeName))
    if (stat == null) watchDirectFrontNode(waitingNodeName) // Nodes has been updated, try again
  }
}
...
def localPending (): Int = localLock.getQueueLength
