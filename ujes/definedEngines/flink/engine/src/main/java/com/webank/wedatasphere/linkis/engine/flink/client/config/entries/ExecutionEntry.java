...
import static com.webank.wedatasphere.linkis.engine.flink.client.config.Environment.*;
...
public class ExecutionEntry extends ConfigEntry {
	...
	private static final String EXECUTION_CHECKPOINT_INTERVAL = "checkpoint-interval";

	private static final String EXECUTION_LOCAL_LIB_PATH = "local-lib-path";
	...
	public long getCheckpointInterval(){
		return properties.getOptionalLong(EXECUTION_CHECKPOINT_INTERVAL)
				.orElseGet(() -> useDefaultValue(EXECUTION_CHECKPOINT_INTERVAL, 0L));
	}

	public List<URL> getLocalLibPath() throws MalformedURLException {
		String localLibPathStringValue = properties.getOptionalString(EXECUTION_LOCAL_LIB_PATH)
				.orElseGet(() -> useDefaultValue(EXECUTION_LOCAL_LIB_PATH, Strings.EMPTY));
		List<URL> localLibUrllist = Lists.newArrayList();
		if (Objects.nonNull(localLibPathStringValue) && localLibPathStringValue.length() > 0) {
			String[] localpaths = localLibPathStringValue.split(",");
			for (String localPath : localpaths) {
				if(StringUtils.isNotEmpty(localPath)){
					URL url = new URL(localPath);
					localLibUrllist.add(url);
				}
			}
		}
		return localLibUrllist;
	}
