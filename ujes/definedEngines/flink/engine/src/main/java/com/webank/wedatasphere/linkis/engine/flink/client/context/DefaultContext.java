...
public class DefaultContext {
	...
	private final Configuration flinkSystemConfig;
	...
	public DefaultContext(Environment defaultEnv,Configuration systemConfiguration,String yarnConfDir, String flinkConfDir, String flinkHome, String distJarPath, String flinkLibRemotePath,String[] providedLibDirsArray,String[] shipDirsArray, List<URL> dependencies) {
		...
		//加载系统级别配置
		this.flinkSystemConfig = GlobalConfiguration.loadConfiguration(this.flinkConfDir);
		if(null!=systemConfiguration){
			this.flinkSystemConfig.addAll(systemConfiguration);
		}

		//设置 flink conf目录
		this.flinkSystemConfig.set(DeploymentOptionsInternal.CONF_DIR, this.flinkConfDir);
		//设置 yarn conf目录
		this.flinkSystemConfig.set(MultipleYarnClusterClientFactory.YARN_CONFIG_DIR, this.yarnConfDir);
		//设置 flink dist jar
		this.flinkSystemConfig.set(YarnConfigOptions.FLINK_DIST_JAR, distJarPath);

		FileSystem.initialize(flinkSystemConfig, PluginUtils.createPluginManagerFromRootFolder(flinkSystemConfig));
		this.flinkConfig = flinkSystemConfig;
	...
	public Configuration getFlinkSystemConfig(){
		return flinkSystemConfig;
	}
	...
	public boolean equals(Object o) {
		...
		return Objects.equals(defaultEnv, context.defaultEnv) &&
			...
			Objects.equals(flinkSystemConfig, context.flinkSystemConfig) &&
	...
	public int hashCode() {
		return Objects.hash(
			defaultEnv, dependencies, flinkConfig, flinkSystemConfig, clusterClientServiceLoader);
	}
}
