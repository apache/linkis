...
public abstract class ClusterDescriptorAdapter<ClusterID> {
	...
	public ClusterDescriptorAdapter(
		...
		this.webInterfaceUrl = getWebInterfaceUrl(executionContext.getFlinkConfig());
	}

	private String getWebInterfaceUrl(Configuration configuration) {
		final String address = configuration.getString(JobManagerOptions.ADDRESS);
		final Integer port = configuration.getInteger(JobManagerOptions.PORT);
		return "http://".concat(address).concat(":").concat(String.valueOf(port));
	}
