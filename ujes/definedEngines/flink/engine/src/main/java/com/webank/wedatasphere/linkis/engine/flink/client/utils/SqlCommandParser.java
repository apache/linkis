...
public final class SqlCommandParser {
	...
	private static Optional<SqlCommandCall> parseStmt(String stmt, boolean isBlinkPlanner) {
		...
		try {
			...
			throw new SqlParseException("Failed to parse statement:" + stmt, e);
		}
		if (sqlNodes.size() != 1) {
		...
		if (node.getKind().belongsTo(SqlKind.QUERY)) {
			...
			SqlCreateFunction sqlCreateFunctionNode = (SqlCreateFunction)node;
			List<String> list = Lists.newArrayList(sqlCreateFunctionNode.getFunctionIdentifier());
			String className = sqlCreateFunctionNode.getFunctionClassName().toString().replace("'","");
			operands = new String[]{list.get(0),className};
		}
	...
	private static final Function<String[],Optional<String[]>> ALL_OPERANDS =
