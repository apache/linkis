...
/**
 * Created by yogafire on 2020/5/19
 */
@EnableResourceManager
class PrestoEntranceManager(resource: ModuleInfo) extends EngineManager with Logging {

  @RegisterResource
  def registerResources(): ModuleInfo = resource
  ...
  override def listEngines(op: EntranceEngine => Boolean): Array[EntranceEngine] =
    idToEngines.values().asScala.filter(e => op(e)).toArray
    ...
    engine.foreach { e =>
      if (!idToEngines.containsKey(e.getId)) idToEngines synchronized {
        if (!idToEngines.containsKey(e.getId)) {
          ...
          info(toString + "：add a new executor => " + e)
        }
      }
    }
  ...
  override def delete(id: Long): Unit = if (idToEngines.containsKey(id)) idToEngines synchronized {
    if (idToEngines.containsKey(id)) {
      ...
      info(s"deleted executor $engine.")
    }
  }
  ...
  override def onEventError(event: EntranceEvent, t: Throwable): Unit = error(s"deal event")
}
